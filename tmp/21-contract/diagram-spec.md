# VERBATIM `## Diagram spec` — from the notes-generator agent specification

This is the authoritative diagram authoring contract. It is reproduced verbatim. Follow it exactly.
...
- **Labels name the real subject matter**, not throwaways. See the topic's own
  labelling rule in your packet.
...
7. **Put the relationship that matters on a straight centre spine.** If the
   ...
   short sub-label in the node and, if it is the point of the whole diagram, an
   annotation panel.
...
- **Section band headers** in `.h2` where the diagram holds two families or
  lanes. Two unlabelled halves read as one confusing whole.
...
each carrying a signature will not fit in three 130-wide boxes. Draw one
panel, one arrow into it, a bold header row, and one left-aligned row per leaf.

**Annotation panels carry the prose.** Facts that are neither node nor edge — a
rule, a bitmask, the reason the spine matters — go in an `#eff6ff` panel with a
bold header and 11px rows, placed in otherwise empty canvas. They are not a
dumping ground: two panels per diagram is the ceiling.

### Skeleton

```svg
<svg viewBox="0 0 900 320" xmlns="http://www.w3.org/2000/svg" role="img"
     aria-label="A prose sentence stating what the picture shows, including the
     relationships a screen reader cannot infer from geometry.">
  <style>
  ...
  <text x="180" y="140" text-anchor="middle" class="lbl node">node title</text>
  <text x="180" y="162" text-anchor="middle" class="lbl body">node body</text>
  <text x="180" y="178" text-anchor="middle" class="lbl sub">sub label</text>

  <path class="flow" d="M 300 148 H 396"/>
  <text x="348" y="140" text-anchor="middle" class="lbl sub">edge gloss</text>

  <rect x="400" y="110" width="300" height="76" rx="6" class="boxR"/>
  <text x="550" y="140" text-anchor="middle" class="lbl node">second node</text>
  <text x="550" y="162" text-anchor="middle" class="lbl body">value 15 = 10 + (10 &gt;&gt; 1)</text>
...
renderable as a picture — it is a grid of values, or it is a list — report it in
your envelope as not renderable with a one-line reason. Do not silently drop it
and never turn it into ASCII art.
