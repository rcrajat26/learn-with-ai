# Writer packet — `switch/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/switch/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `switch/01-basics.md` |
| Subject | switch |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.16 — `switch` expressions and statements |
| Leaves | 1.16.1–1.16.18 (18 leaves) |
| Primary concepts | switch expressions and the arrow form; `yield` and why `return` is illegal; exhaustiveness in expressions and in Java 21 pattern statements; the colon form and fall-through; the permitted selector types; the `default`-in-an-enum-switch trade-off |
| Diagrams | D-066, D-067, D-068 |
| Examples (QuizStakes slice) | Dispatching on `RestrictionType`; a colon switch over restriction sources with a missing `break` |
| Previous | `pattern-matching/03-internals-pattern-matching.md` |
| Next | `switch/03-internals-switch-compilation.md` |
| Target size | **940 lines**, and anything from 658 to 1269 is fine |
...
- **D-066 — Switch forms compared** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-066** — Switch forms compared` on the line beneath it. It must contain every row, column and value named here: Rows: colon statement, arrow statement, colon expression with `yield`, arrow expression. Columns: fall-through, produces a value, `break` allowed, `yield` allowed, `return` allowed, exhaustiveness required, mixing allowed with the others. The QuizStakes example is dispatching on `RestrictionType`
- **D-067 — Fall-through, and how the arrow form makes it unwritable** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-067 — Fall-through, and how the arrow form makes it unwritable](../diagrams/D-067-fall-through-arrow-form.svg)
  Beneath each embed put the caption line `**D-067** — Fall-through, and how the arrow form makes it unwritable`. The diagram shows: Left: a colon switch over restriction sources with a missing `break`, the execution path falling through two arms drawn as one continuous arrow, and the wrong outcome named. Right: the same logic in arrow form, one arm executed, no `break` written or needed
- **D-068 — Exhaustive enum switch expression versus one with `default`** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-068 — Exhaustive enum switch expression versus one with `default`](../diagrams/D-068-exhaustive-enum-switch-expression.svg)
  Beneath each embed put the caption line `**D-068** — Exhaustive enum switch expression versus one with `default``. The diagram shows: Left: an exhaustive switch expression over `RestrictionType` with no `default`; adding `DORMANT_FROZEN` to the enum produces a compile error, quoted. Right: the same with `default ->`; adding the constant compiles and silently takes the default. A third panel shows the synthetic default the compiler still emits, throwing `IncompatibleClassChangeError` when the enum changed after separate compilation
...
```
path: switch/01-basics.md
lines: <wc -l of the file you wrote>
leaves: 1.16.1–1.16.18
