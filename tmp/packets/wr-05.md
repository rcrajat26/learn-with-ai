# Writer packet — `functional-interfaces/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/functional-interfaces/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `functional-interfaces/01-basics.md` |
| Subject | functional interfaces |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.2 — Functional interfaces |
| Leaves | 1.2.1–1.2.20 (20 leaves) |
| Primary concepts | the SAM definition and the `Object`-method exclusion; the six core shapes and their narrowings; the 43-interface inventory and its naming scheme; why the primitive specialisations exist; the shapes the JDK withholds |
| Diagrams | D-005, D-006, D-007, D-008 |
| Examples (QuizStakes slice) | `Function<LedgerEntry, Money>`, `Predicate<Restriction>`, `Supplier<IdempotencyKey>`; a domain-named `StakeRule` beating `Function<Reservation, Money>`; §15 Example Bank rows on restriction evaluation |
| Previous | `platform-and-releases/04-internals-observability.md` |
| Next | `lambdas/01-basics.md` |
| Target size | **972 lines**, and anything from 680 to 1312 is fine |
...
These are the **20 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
```
path: functional-interfaces/01-basics.md
