# Writer packet — `library-additions/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/library-additions/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `library-additions/01-basics.md` |
| Subject | library additions |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.20 — The library additions, 9 → 21 |
| Leaves | 1.20.1–1.20.24 (24 leaves) |
| Primary concepts | the collection factories and their null hostility; the Java 9 stream and `Optional` additions; the Java 11 `String` and `Files` surface and `HttpClient`; `teeing`; `Stream.toList` and `mapMulti`; `RandomGenerator`; JEP 400's UTF-8 default; sequenced collections and the retrofit; `reversed()` as a view |
| Diagrams | D-084, D-085, D-086 |
| Examples (QuizStakes slice) | A `LinkedHashMap` of restriction keys reversed as a view; `Map.of` iteration order changing between JVM runs while listing gates |
| Previous | `structured-concurrency/03-internals.md` |
| Next | `cost-model/02-master-tables.md` |
...
```
path: library-additions/01-basics.md
lines: <wc -l of the file you wrote>
leaves: 1.20.1–1.20.24
