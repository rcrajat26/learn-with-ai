# Illustrator packet ill-16 — topic 04 Modern Java
...
- **Target filename:** `D-080-structured-task-scope-tree.svg`
- **Type:** hierarchy
- **Syllabus leaf:** 1.19.1–1.19.3, 3.15.1
...
- **Target filename:** `D-081-shutdownonfailure-versus-completablefuture-allof.svg`
- **Type:** timeline
- **Syllabus leaf:** 1.19.5, 1.19.11, 2.13.4
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Two lanes on one time axis. Lane 1, `ShutdownOnFailure`: the watchlist call fails at 1.4 s, the identity call is interrupted, `join()` returns, `throwIfFailed()` rethrows. Lane 2, `allOf`: the same failure, the identity call keeps running past the block's end, marked "orphan", still holding its connection. Both end states labelled

### D-082 — `ShutdownOnSuccess` as a hedge

- **Target filename:** `D-082-shutdownonsuccess-hedge.svg`
- **Type:** timeline
- **Syllabus leaf:** 1.19.6, 2.13.2
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Two replicas of the watchlist provider, one responding at 1.4 s and one at 25 s. One axis, two lanes: the fast one completes, the scope cancels the slow one at 1.4 s, the total latency marked. A comparison line shows the un-hedged p99 of 25 s

### D-083 — `Subtask` states and the illegal calls

- **Target filename:** `D-083-subtask-states-illegal-calls.svg`
- **Type:** state-transition
- **Syllabus leaf:** 1.19.8, 1.19.9, 3.15.2, 3.15.3
