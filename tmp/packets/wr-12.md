# Writer packet — `streams/02-sources.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/02-sources.md
```
...
| Field | Value |
|---|---|
| File | `streams/02-sources.md` |
| Subject | streams |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.6 — Stream sources |
| Leaves | 1.6.1–1.6.18 (18 leaves) |
| Primary concepts | every stream source and its guarantees; `IntStream.range` as the best-splitting source; `Stream.iterate`'s two forms; `Stream.concat`'s left-deep tree; `StreamSupport` as the escape hatch; the sources that need closing |
| Diagrams | D-023, D-024 |
| Examples (QuizStakes slice) | `ledgerEntries.stream()`, `IntStream.range(0, 2_800_000)` over a day of stake reservations, `Files.lines(paymentRunFile)`, and a hand-written JDBC bridge for `ResultSet` |
| Previous | `streams/01-basics-the-model.md` |
| Next | `streams/03-intermediate-operations.md` |
| Target size | **932 lines**, and anything from 652 to 1258 is fine |
...
- **D-023 — The stream source catalogue** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-023** — The stream source catalogue` on the line beneath it. It must contain every row, column and value named here: One row per source. Columns: since which release, finite/infinite, ordered, `SIZED`/`SUBSIZED`, split quality (excellent/good/serial), needs closing, and the QuizStakes use (`ledgerEntries.stream()`, `IntStream.range(0, 2_800_000)`, `Files.lines(paymentRunFile)`)
- **D-024 — `Stream.concat` in a loop builds a left-deep tree** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-024 — `Stream.concat` in a loop builds a left-deep tree](../diagrams/D-024-stream-concat-loop-builds.svg)
  Beneath each embed put the caption line `**D-024** — `Stream.concat` in a loop builds a left-deep tree`. The diagram shows: Left: five successive `concat` calls drawn as a left-leaning binary tree five levels deep, with the recursion depth labelled and `StackOverflowError` at traversal. Right: the same five sources collected into a `List` and flat-mapped, one level deep. Depths written on both
