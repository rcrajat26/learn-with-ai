# Writer packet — `records/03-internals-records.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/records/03-internals-records.md
```
...
| Field | Value |
|---|---|
| File | `records/03-internals-records.md` |
| Subject | records |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.9 — Record internals |
| Leaves | 3.9.1–3.9.14 (14 leaves) |
| Primary concepts | the `Record` class-file attribute and its `record_component_info` entries; `ObjectMethods.bootstrap` behind the three generated methods; why the `hashCode` algorithm is unspecified; the compact-constructor desugaring in `javap`; record serialization and the ignored hooks; blocked `setAccessible` |
| Diagrams | D-149, D-150, D-151 |
| Examples (QuizStakes slice) | `javap -v` on `StakeSplit`, with the component-name string `"bonusPortion;cashPortion"` and one `MethodHandle` getter per component |
| Previous | `records/02-in-practice.md` |
| Next | `sealed-types/01-basics.md` |
| Target size | **892 lines**, and anything from 624 to 1204 is fine |
...
- **D-149 — The `Record` class-file attribute** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-149 — The `Record` class-file attribute](../diagrams/D-149-record-class-file-attribute.svg)
  Beneath each embed put the caption line `**D-149** — The `Record` class-file attribute`. The diagram shows: The class file of `StakeSplit` with its `Record` attribute expanded: two `record_component_info` entries, each with a name index, a descriptor index and its own attributes (`Signature`, `RuntimeVisibleAnnotations`, `RuntimeVisibleTypeAnnotations`). The `private final` fields, the public accessors, and `extends java.lang.Record` (itself abstract, declaring abstract `equals`/`hashCode`/`toString`) all labelled
- **D-150 — `ObjectMethods.bootstrap` behind `equals`, `hashCode` and `toString`** · type `flowchart` · the file(s) already exist; embed exactly:
...
```
path: records/03-internals-records.md
