# Illustrator packet ill-03 — topic 04 Modern Java
...
- **Target filename:** `D-013-loop-variable-capturable.svg`
- **Type:** before-after
- **Syllabus leaf:** 1.3.15
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Left: `for (int i = 0; i < 3; i++)` — one variable, reassigned, so `i` is not effectively final and the lambda does not compile; the single slot drawn once with three values over time. Right: `for (Reservation r : reservations)` — a fresh variable per iteration, three separate slots, three lambdas each capturing its own. The compile error text quoted on the left

### D-016 — Unbound receiver becomes the first parameter

- **Target filename:** `D-016-unbound-receiver-becomes-first.svg`
- **Type:** before-after
- **Syllabus leaf:** 1.4.4
