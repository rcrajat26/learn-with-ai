# Writer packet — `build-it/07-diagnostic-harnesses.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/build-it/07-diagnostic-harnesses.md
```
...
| Field | Value |
|---|---|
| File | `build-it/07-diagnostic-harnesses.md` |
| Subject | build it |
| Part / tier | Part 4 — BUILD IT |
| Syllabus sections | §4.8 — Diagnostic harnesses |
| Leaves | 4.8.1–4.8.12 (12 leaves) |
...
| Diagrams | D-178 |
| Examples (QuizStakes slice) | Every harness run over QuizStakes data: 2.8M reservations, 95k deposits, the `Verdict` hierarchy, the ledger SQL text block |
| Previous | `build-it/06-filling-the-21-gaps.md` |
| Next | `90-interview-basics.md` |
...
- **D-178 — The fifteen puzzlers and their mechanisms** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-178** — The fifteen puzzlers and their mechanisms` on the line beneath it. It must contain every row, column and value named here: One row per puzzler: `peek` elision, stream reuse, `toList` immutability, `toMap` null value, `groupingBy` null key, `orElse` eagerness, `Optional.empty()` identity, `var` diamond, record array `equals`, pattern-switch NPE, text-block indentation, bound method-reference NPE, `allMatch` on empty, `IntStream.sum` overflow, parallel `forEach` corruption. Columns: what a reader predicts, what actually happens, the mechanism, and the syllabus leaf
...
```
path: build-it/07-diagnostic-harnesses.md
