# Writer packet — `var/02-in-practice.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/var/02-in-practice.md
```
...
| Field | Value |
|---|---|
| File | `var/02-in-practice.md` |
| Subject | var |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.7 — `var` in practice |
| Leaves | 2.7.1–2.7.10 (10 leaves) |
| Primary concepts | a style policy defensible in review; the cases where `var` clearly wins; the interface-versus-implementation trap; numeric-literal width; `var` in lambda parameters; what refactoring does to a `var` local |
| Diagrams | D-107 |
| Examples (QuizStakes slice) | Iterating `Map.Entry<RestrictionKey, Restriction>`; a `Map<String, List<Map<String, Integer>>>` of per-rail counts |
| Previous | `var/01-basics.md` |
| Next | `var/03-internals-inference.md` |
...
- **D-107 — A `var` policy you can defend in review** · type `decision-tree` · the file(s) already exist; embed exactly:
    ![D-107 — A `var` policy you can defend in review](../diagrams/D-107-var-policy-can-defend.svg)
  Beneath each embed put the caption line `**D-107** — A `var` policy you can defend in review`. The diagram shows: Root: "does the initialiser already name the type?" Yes → `var` is fine, with the builder, try-with-resources and `Map.Entry` cases as leaves. No → write the type, with the opaque-factory, accumulator-width and interface-vs-implementation leaves. Each "no" leaf carries the concrete failure: `var total = 0` overflowing, `var list = new ArrayList<String>()` pinning the local's type to `ArrayList`
