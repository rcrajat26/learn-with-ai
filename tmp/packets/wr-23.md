# Writer packet — `collectors/02-in-anger.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/collectors/02-in-anger.md
```
...
| Field | Value |
|---|---|
| File | `collectors/02-in-anger.md` |
| Subject | collectors |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.5 — Collectors in anger |
| Leaves | 2.5.1–2.5.14 (14 leaves) |
| Primary concepts | multi-level grouping and reading the nested type; `filtering` versus a pre-`filter`; choosing the map implementation; `toMap` merge strategies; `teeing`; a bounded top-N collector; a boxing-free statistics collector; three routes to an immutable result |
| Diagrams | D-103, D-104 |
| Examples (QuizStakes slice) | Top-3 withdrawals by amount (180, 260, 92) merged across two leaves; grouping card deposits by rail where one rail has nothing above 100 |
| Previous | `collectors/01-basics-b.md` |
| Next | `collectors/03-internals-collectors.md` |
| Target size | **884 lines**, and anything from 618 to 1193 is fine |
...
- **D-103 — `filtering(p, toList())` versus `filter(p)` before `groupingBy`** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-103 — `filtering(p, toList())` versus `filter(p)` before `groupingBy`](../diagrams/D-103-filtering-p-tolist-versus.svg)
  Beneath each embed put the caption line `**D-103** — `filtering(p, toList())` versus `filter(p)` before `groupingBy``. The diagram shows: Grouping card deposits by rail where one rail has no deposit above 100. Left, `filter` before `groupingBy`: that rail's key is absent from the map entirely. Right, `filtering` as a downstream: the key is present with an empty list. Both result maps drawn key by key
- **D-104 — A top-N collector's combiner** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-104 — A top-N collector's combiner](../diagrams/D-104-top-n-collector-s.svg)
  Beneath each embed put the caption line `**D-104** — A top-N collector's combiner`. The diagram shows: Top-3 withdrawals by amount over two parallel leaves. Frame 1: each leaf maintains a bounded `PriorityQueue` of size 3, contents shown. Frame 2: the combiner merges the two heaps and re-bounds to 3 — the discarded elements marked. Frame 3: the finisher sorts descending. Actual withdrawal amounts (180, 260, 92) used
...
```
path: collectors/02-in-anger.md
