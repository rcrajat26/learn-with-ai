# Writer packet — `build-it/01-functional-toolkit.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/build-it/01-functional-toolkit.md
```
...
| Field | Value |
|---|---|
| File | `build-it/01-functional-toolkit.md` |
| Subject | build it |
| Part / tier | Part 4 — BUILD IT |
| Syllabus sections | §4.1 — A functional toolkit from scratch |
| Leaves | 4.1.1–4.1.8 (8 leaves) |
| Primary concepts | `MyFunction` and `MyPredicate` with composition; `CheckedFunction` plus `unchecked`/`sneaky`; a `Result<T,E>` sealed type; a memoising decorator and the `computeIfAbsent` recursion deadlock; curry/partial; `TriFunction` |
| Diagrams | — |
| Examples (QuizStakes slice) | Composition over `Money` fee-then-rounding on the 3.33 stake; an `IOException`-throwing payout-file read routed through `Result` |
| Previous | `which-construct/02-which-construct.md` |
| Next | `build-it/02-mystream.md` |
| Target size | **796 lines**, and anything from 557 to 1074 is fine |
...
```
path: build-it/01-functional-toolkit.md
