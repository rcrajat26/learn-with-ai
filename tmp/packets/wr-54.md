# Writer packet — `which-construct/02-which-construct.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/which-construct/02-which-construct.md
```
...
| Field | Value |
|---|---|
| File | `which-construct/02-which-construct.md` |
| Subject | which construct |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.15 — Which construct |
| Leaves | 2.15.1–2.15.10 (10 leaves) |
| Primary concepts | the ten construct decisions, each with a default answer and the condition that overrides it |
| Diagrams | D-124 |
| Examples (QuizStakes slice) | Each decision resolved on a real QuizStakes call: the payment-run batch, the assessment fan-out, the restriction evaluation, the ledger projection |
| Previous | `cost-model/02-master-tables.md` |
| Next | `build-it/01-functional-toolkit.md` |
...
- **D-124 — The which-construct index** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-124** — The which-construct index` on the line beneath it. It must contain every row, column and value named here: One row per question in §2.15. Columns: the question, the default answer, the condition that overrides the default, and the section of these notes that argues it
...
```
path: which-construct/02-which-construct.md
lines: <wc -l of the file you wrote>
leaves: 2.15.1–2.15.10
