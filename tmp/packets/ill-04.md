# Illustrator packet ill-04 — topic 04 Modern Java
...
- **Target filename:** `D-019-fusion-element-through-whole.svg`
- **Type:** before-after
- **Syllabus leaf:** 1.5.5, 3.3.11
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Left, the wrong mental model: three passes over the whole collection with two intermediate lists materialised, sizes labelled. Right, the real model: one element at a time entering `filter`, then `map`, then the collector, with a numbered trace for the first three deposits (65, 480, 65) and no intermediate collection anywhere

### D-021 — A stream is consumed once

- **Target filename:** `D-021-stream-consumed-once.svg`
- **Type:** state-transition
- **Syllabus leaf:** 1.5.13, 3.3.12
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Three states — unconsumed, linked (an intermediate op attached), consumed/closed — with the transitions labelled by the call that causes them and the two exact exception messages on the illegal edges: `"stream has already been operated upon or closed"` and `"source already consumed or closed"`

### D-022 — Which streams must be closed

- **Target filename:** `D-022-streams-must-closed.svg`
- **Type:** decision-tree
- **Syllabus leaf:** 1.5.14, 1.5.15, 1.6.10
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Root: "does the source hold an OS resource?" Yes branch lists `Files.lines`, `Files.walk`, `Files.list`, `Files.find`, `Files.newDirectoryStream` → try-with-resources required, with the file-descriptor leak as the symptom. No branch lists `Collection.stream`, `Arrays.stream`, `IntStream.range` → closing is a no-op. `onClose(Runnable)` shown on the yes branch

### D-024 — `Stream.concat` in a loop builds a left-deep tree

- **Target filename:** `D-024-stream-concat-loop-builds.svg`
- **Type:** before-after
- **Syllabus leaf:** 1.6.7
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Left: five successive `concat` calls drawn as a left-leaning binary tree five levels deep, with the recursion depth labelled and `StackOverflowError` at traversal. Right: the same five sources collected into a `List` and flat-mapped, one level deep. Depths written on both
