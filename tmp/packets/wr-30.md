# Writer packet — `var/03-internals-inference.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/var/03-internals-inference.md
```
...
| Field | Value |
|---|---|
| File | `var/03-internals-inference.md` |
| Subject | var |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.8 — `var` and inference internals |
| Leaves | 3.8.1–3.8.8 (8 leaves) |
| Primary concepts | standalone type plus upward projection; the `LocalVariableTable` as the only trace; why a field or parameter could never work; diamond inference with no target type; anonymous-class initialisers |
| Diagrams | D-147, D-148 |
| Examples (QuizStakes slice) | `List<? extends Money> amounts; var first = amounts.get(0);` projecting the capture variable away to `Money` |
| Previous | `var/02-in-practice.md` |
| Next | `records/01-basics-a.md` |
...
- **D-147 — Upward projection** · type `step-sequence, 3 frames` · authored as **4 frame files**, and you must embed **all of them, in order**, each with its own caption line naming the frame · the file(s) already exist; embed exactly:
...
```
path: var/03-internals-inference.md
