# Writer packet — `switch/03-internals-switch-compilation.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/switch/03-internals-switch-compilation.md
```
...
| Field | Value |
|---|---|
| File | `switch/03-internals-switch-compilation.md` |
| Subject | switch |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.12 — `switch` compilation |
| Leaves | 3.12.1–3.12.8 (8 leaves) |
| Primary concepts | `tableswitch` versus `lookupswitch` and the density heuristic; the two-stage `String` switch; `$SwitchMap` protecting a separately compiled enum switch; the arrow form compiling identically; the operand stack at the join point; the synthetic default in an exhaustive enum switch expression |
| Diagrams | D-155, D-156, D-157 |
| Examples (QuizStakes slice) | `$SwitchMap$RestrictionType` contents shown, and the enum reordered without recompiling the switch |
| Previous | `switch/01-basics.md` |
| Next | `text-blocks/01-basics.md` |
| Target size | **820 lines**, and anything from 574 to 1107 is fine |
...
- **D-155 — `tableswitch` versus `lookupswitch`** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-155 — `tableswitch` versus `lookupswitch`](../diagrams/D-155-tableswitch-versus-lookupswitch.svg)
  Beneath each embed put the caption line `**D-155** — `tableswitch` versus `lookupswitch``. The diagram shows: Left: dense case labels compiling to `tableswitch` with the jump table drawn. Right: sparse labels compiling to `lookupswitch` with the key/offset pairs drawn. Beneath, the two-stage `String` switch: `lookupswitch` on `hashCode`, `equals` to confirm, then a second switch on a synthetic index — worked on a restriction-type name
- **D-156 — `$SwitchMap` protects a separately compiled enum switch** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-156 — `$SwitchMap` protects a separately compiled enum switch](../diagrams/D-156-switchmap-protects-separately-compiled.svg)
  Beneath each embed put the caption line `**D-156** — `$SwitchMap` protects a separately compiled enum switch`. The diagram shows: Left: a switch over `RestrictionType` compiled with the synthetic `$SwitchMap$RestrictionType` `int[]` mapping `ordinal()` to a stable case index — the array contents shown. Right: the enum reordered and recompiled without recompiling the switch — the map absorbs the change and the correct arm still runs. The failure that would occur without it drawn as a third panel
- **D-157 — The synthetic default in an exhaustive enum switch expression** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-157 — The synthetic default in an exhaustive enum switch expression](../diagrams/D-157-synthetic-default-exhaustive-enum.svg)
  Beneath each embed put the caption line `**D-157** — The synthetic default in an exhaustive enum switch expression`. The diagram shows: Left: the source with no `default`. Right: the `javap -c` output with the synthetic default arm that throws `IncompatibleClassChangeError`. Beneath, the scenario that reaches it: a constant added to the enum after the switch's class was compiled. A VERSION TRAP note that the thrown type has changed shape across releases
...
```
path: switch/03-internals-switch-compilation.md
lines: <wc -l of the file you wrote>
leaves: 3.12.1–3.12.8
