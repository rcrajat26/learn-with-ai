# Writer packet — `collectors/01-basics-a.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/collectors/01-basics-a.md
```
...
| Field | Value |
|---|---|
| File | `collectors/01-basics-a.md` |
| Subject | collectors |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.10 — Collectors |
| Leaves | 1.10.1–1.10.16 (16 leaves) |
| Primary concepts | the five-function `Collector` contract and the three characteristics; the `toX` family; `toMap`'s duplicate-key and null-value failures; `joining`; the summing/averaging/summarizing family and Kahan summation; `mapping`/`filtering`/`flatMapping`/`collectingAndThen` |
| Diagrams | D-039, D-040 |
| Examples (QuizStakes slice) | Collecting 95k card deposits by rail; `toMap` on `(ClientId, Money)` with a duplicate identity; `summingDouble` over deposits averaging 65 |
| Previous | `streams/10-internals-parallel-execution.md` |
| Next | `collectors/01-basics-b.md` |
| Target size | **908 lines**, and anything from 635 to 1225 is fine |
...
- **D-039 — The `Collector` contract's five functions** · type `step-sequence, 4 frames` · authored as **4 frame files**, and you must embed **all of them, in order**, each with its own caption line naming the frame · the file(s) already exist; embed exactly:
  ...
  Beneath each embed put the caption line `**D-039** — The `Collector` contract's five functions`. The diagram shows: A `groupingBy(deposit -> deposit.rail(), counting())` over card and bank deposits. Frame 1: `supplier()` creates the container. Frame 2: `accumulator()` folds each element in. Frame 3: `combiner()` merges two containers in the parallel case. Frame 4: `finisher()` transforms to the result type, with `IDENTITY_FINISH` shown as the skip path. The three `Characteristics` values listed in a legend
- **D-040 — Collector inventory** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-040** — Collector inventory` on the line beneath it. It must contain every row, column and value named here: One row per collector factory across all 30 names and 54 overloads. Columns: version added, result type, mutability of the result, null policy, characteristics, parallel behaviour, and the one trap if it has one
...
```
path: collectors/01-basics-a.md
lines: <wc -l of the file you wrote>
leaves: 1.10.1–1.10.16
