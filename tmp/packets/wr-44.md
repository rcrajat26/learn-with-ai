# Writer packet — `text-blocks/02-in-practice.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/text-blocks/02-in-practice.md
```
...
| Field | Value |
|---|---|
| File | `text-blocks/02-in-practice.md` |
| Subject | text blocks |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.11 — Text blocks in practice |
| Leaves | 2.11.1–2.11.8 (8 leaves) |
| Primary concepts | SQL with bound parameters rather than interpolation; JSON fixtures with `formatted`; regex where the text block loses; trailing-newline discipline; text blocks in annotations and `case` labels; the absence of interpolation in Java 21 |
| Diagrams | D-115 |
| Examples (QuizStakes slice) | The ledger-balance SQL as a text block with `?` placeholders; a `DEP-301 CAPTURED` webhook JSON fixture |
| Previous | `text-blocks/01-basics.md` |
| Next | `text-blocks/03-internals-compilation.md` |
...
```
path: text-blocks/02-in-practice.md
lines: <wc -l of the file you wrote>
leaves: 2.11.1–2.11.8
