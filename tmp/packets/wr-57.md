# Writer packet — `build-it/03-collectors-and-myoptional.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/build-it/03-collectors-and-myoptional.md
```
...
| Field | Value |
|---|---|
| File | `build-it/03-collectors-and-myoptional.md` |
| Subject | build it |
| Part / tier | Part 4 — BUILD IT |
| Syllabus sections | §4.3, §4.4 — Collectors from scratch, `MyOptional` |
| Leaves | 4.3.1–4.3.7, 4.4.1–4.4.6 (13 leaves) |
| Primary concepts | `MyCollector` and the five-function contract; `toList`/`joining`/`groupingBy` with correct combiners; a bounded top-N and a boxing-free statistics collector; a `CONCURRENT` collector harness; `MyOptional` with the shared `EMPTY`; eager-versus-lazy and allocation harnesses |
| Diagrams | — |
| Examples (QuizStakes slice) | Top-3 withdrawals (180, 260, 92); a `long[]` statistics accumulator over 2.8M stake minor-unit amounts |
| Previous | `build-it/02-mystream.md` |
| Next | `build-it/04-records-sealed-patterns.md` |
| Target size | **856 lines**, and anything from 599 to 1155 is fine |
...
```
path: build-it/03-collectors-and-myoptional.md
lines: <wc -l of the file you wrote>
leaves: 4.3.1–4.3.7, 4.4.1–4.4.6
