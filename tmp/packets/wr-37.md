# Writer packet — `sealed-types/03-internals-sealed.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/sealed-types/03-internals-sealed.md
```
...
| Field | Value |
|---|---|
| File | `sealed-types/03-internals-sealed.md` |
| Subject | sealed types |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.10 — Sealed internals |
| Leaves | 3.10.1–3.10.8 (8 leaves) |
| Primary concepts | the `PermittedSubclasses` attribute and the absence of `ACC_SEALED`; `non-sealed` emitting nothing; load-time enforcement surviving bytecode manipulation; the same-module check; narrowing reference conversion; the separate-compilation hazard |
| Diagrams | D-152 |
| Examples (QuizStakes slice) | The `Verdict` class file with four constant-pool indices, and a bytecode-manipulated fifth subclass rejected at load time |
| Previous | `sealed-types/02-data-oriented-programming.md` |
| Next | `pattern-matching/01-basics.md` |
| Target size | **804 lines**, and anything from 562 to 1085 is fine |
...
- **D-152 — `PermittedSubclasses` is enforced at load time** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-152 — `PermittedSubclasses` is enforced at load time](../diagrams/D-152-permittedsubclasses-enforced-load-time.svg)
  Beneath each embed put the caption line `**D-152** — `PermittedSubclasses` is enforced at load time`. The diagram shows: Frame 1: the `Verdict` class file with its `PermittedSubclasses` attribute listing four constant-pool indices; no `ACC_SEALED` flag exists, labelled. Frame 2: a bytecode-manipulated fifth subclass produced at runtime. Frame 3: the JVM's load-time check failing it. A note that `non-sealed` emits no attribute at all
...
```
path: sealed-types/03-internals-sealed.md
lines: <wc -l of the file you wrote>
leaves: 3.10.1–3.10.8
