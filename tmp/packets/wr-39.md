# Writer packet — `pattern-matching/02-in-anger.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/pattern-matching/02-in-anger.md
```
...
| Field | Value |
|---|---|
| File | `pattern-matching/02-in-anger.md` |
| Subject | pattern matching |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.10 — Pattern matching in anger |
| Leaves | 2.10.1–2.10.12 (12 leaves) |
| Primary concepts | refactoring an `instanceof` chain step by step; record deconstruction replacing getter-plus-condition; guards versus nested switches; naming the total pattern; handling null explicitly; migration risk and exhaustiveness drift; the `typeSwitch` cost model; the readability limit |
| Diagrams | D-113, D-114 |
| Examples (QuizStakes slice) | The `Verdict` `if`/`else if` chain converted in four steps; a fifth `Verdict` case added and only the hierarchy redeployed |
| Previous | `pattern-matching/01-basics.md` |
| Next | `pattern-matching/03-internals-pattern-matching.md` |
...
- **D-113 — Refactoring an `instanceof` chain into a pattern switch** · type `step-sequence, 4 frames` · the file(s) already exist; embed exactly:
    ![D-113 — Refactoring an `instanceof` chain into a pattern switch](../diagrams/D-113-refactoring-instanceof-chain-pattern.svg)
  Beneath each embed put the caption line `**D-113** — Refactoring an `instanceof` chain into a pattern switch`. The diagram shows: Frame 1: the original `if`/`else if` chain of `instanceof` + cast over `Verdict`. Frame 2: type patterns replacing the casts. Frame 3: converted to a pattern switch, `default` still present. Frame 4: `default` removed once the type is sealed, so exhaustiveness is checked. Each frame states what the compiler now guarantees that it did not before
- **D-114 — Exhaustiveness drift after a partial redeploy** · type `timeline` · the file(s) already exist; embed exactly:
    ![D-114 — Exhaustiveness drift after a partial redeploy](../diagrams/D-114-exhaustiveness-drift-after-partial.svg)
  Beneath each embed put the caption line `**D-114** — Exhaustiveness drift after a partial redeploy`. The diagram shows: Three points: (1) both the sealed hierarchy and the switch site compiled together, exhaustive; (2) a fifth `Verdict` case added and only the hierarchy recompiled and deployed; (3) at runtime the switch matches nothing and throws `MatchException` (or `IncompatibleClassChangeError`). The class-file states at each point drawn, and the note that this is not a link error
...
```
path: pattern-matching/02-in-anger.md
lines: <wc -l of the file you wrote>
leaves: 2.10.1–2.10.12
