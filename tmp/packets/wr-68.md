# Writer packet — `94-interview-questions-c.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/94-interview-questions-c.md
```
...
| Field | Value |
|---|---|
| File | `94-interview-questions-c.md` |
| Subject | 94 interview questions c.md |
| Part / tier | Part 5 — INTERVIEW |
| Syllabus sections | §5.1 — The questions, with the answer shape |
| Leaves | 5.1.65–5.1.95 (31 leaves) |
| Primary concepts | questions 5.1.65–5.1.95 with the full answer shape: sealed types, pattern matching, switch, text blocks, virtual threads, structured concurrency, sequenced collections |
| Diagrams | — |
| Examples (QuizStakes slice) | Every answer grounded in the `Verdict` hierarchy, the pinning JDBC driver and the assessment fan-out |
| Previous | `94-interview-questions-b.md` |
| Next | `95-traps-drills-and-checklist.md` |
| Target size | **994 lines**, and anything from 695 to 1341 is fine |
...
These are the **31 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- None. This file carries no diagram from the manifest. Do not invent one, do not embed one belonging to another file, and do not draw with ASCII characters. Where you need a visual, a Markdown table is the correct substitute.



Each of your leaves is one interview question. Answer **every one** with the answer shape a candidate would actually say out loud — the 60-to-90-second spoken answer, not a hint and not a bullet list of API names. Structure each as: `### <question, verbatim from the leaf>`, then the spoken answer, then a `**Interview:**` line giving the one-sentence version for when time is short. Where a question has a 30-second and a 5-minute answer, give both and label them. This file's `## Self-test` section is still required and is separate from the questions themselves — use it for the traps the questions did not cover.
...
```
path: 94-interview-questions-c.md
lines: <wc -l of the file you wrote>
leaves: 5.1.65–5.1.95
