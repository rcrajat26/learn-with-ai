# Writer packet — `streams/05-primitive-streams.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/05-primitive-streams.md
```
...
| Field | Value |
|---|---|
| File | `streams/05-primitive-streams.md` |
| Subject | streams |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.9 — Primitive streams |
| Leaves | 1.9.1–1.9.16 (16 leaves) |
| Primary concepts | the three primitive streams and the conversions between the four shapes; why there is no `CharStream`; `OptionalInt`'s deliberately thinner API; `IntStream.sum()` overflow; the memory arithmetic for boxed versus primitive |
| Diagrams | D-036, D-037, D-038 |
| Examples (QuizStakes slice) | 2.8M stake amounts in minor units as `int[]` versus `List<Integer>`; the `int` total wrapping past 2 147 483 647 |
| Previous | `streams/04-terminal-operations.md` |
| Next | `streams/06-cost-model.md` |
| Target size | **916 lines**, and anything from 641 to 1236 is fine |
...
- **D-036 — The four stream shapes and the conversions between them** · type `hierarchy` · the file(s) already exist; embed exactly:
    ![D-036 — The four stream shapes and the conversions between them](../diagrams/D-036-four-stream-shapes-conversions.svg)
  Beneath each embed put the caption line `**D-036** — The four stream shapes and the conversions between them`. The diagram shows: Four boxes — `Stream<T>`, `IntStream`, `LongStream`, `DoubleStream` — with every conversion arrow labelled by its method (`mapToInt`, `mapToObj`, `boxed`, `asLongStream`, `asDoubleStream`, `mapToLong`, `mapToDouble`). A note box: no `CharStream`, `BooleanStream` or `FloatStream`, and which primitives widen into which
- **D-037 — `int[]` versus `List<Integer>` for 2.8M stake amounts** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-037 — `int[]` versus `List<Integer>` for 2.8M stake amounts](../diagrams/D-037-int-versus-list-integer.svg)
  Beneath each embed put the caption line `**D-037** — `int[]` versus `List<Integer>` for 2.8M stake amounts`. The diagram shows: Top: `int[] stakeMinorUnits = new int[2_800_000]` — 16-byte header plus 11.2 MB contiguous. Bottom: the boxed equivalent — a 24-byte list, a 16-byte array header, 11.2 MB of 4-byte references, and 2.8M × 16-byte `Integer` objects. Both totals and the ratio written out
- **D-038 — `IntStream.sum()` overflows silently** · type `step-sequence, 3 frames` · authored as **3 frame files**, and you must embed **all of them, in order**, each with its own caption line naming the frame · the file(s) already exist; embed exactly:
...
```
path: streams/05-primitive-streams.md
