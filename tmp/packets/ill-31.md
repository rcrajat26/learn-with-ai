# Illustrator packet ill-31 — topic 04 Modern Java
...
- **Target filename:** `D-158-text-block-constant-folded.svg`
- **Type:** before-after
- **Syllabus leaf:** 3.13.1, 3.13.4, 3.13.6
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Left: the source text block. Right: the `javap -v` constant pool showing a single `CONSTANT_String_info` with the final, already-stripped content — nothing of the algorithm surviving to runtime. Beneath: a text block and an equal string literal compared with `==`, both pointing at the same interned constant, with the caution that this is not a habit to build

### D-159 — The three layers of a virtual thread

- **Target filename:** `D-159-three-layers-virtual-thread.svg`
- **Type:** hierarchy
- **Syllabus leaf:** 3.14.1, 3.14.11
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** `java.lang.VirtualThread` on top, `jdk.internal.vm.Continuation` beneath it, the FIFO `ForkJoinPool` scheduler beneath that, and platform carrier threads at the bottom. `Thread.currentThread()` drawn returning the `VirtualThread`, with the carrier marked reachable only through internal API

### D-160 — Stack chunks live on the heap

- **Target filename:** `D-160-stack-chunks-live-heap.svg`
- **Type:** memory-layout
- **Syllabus leaf:** 3.14.2, 3.14.3, 3.14.10, 2.12.12
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** One carrier's native stack with three mounted frames; the heap holding `StackChunk` objects for the unmounted threads; arrows for the copy in both directions labelled "mount" and "unmount", with the lazy/partial copying noted. The heap arithmetic for 1,000,000 virtual threads written out and contrasted with 1,000,000 × 1 MB of reserved platform stack

### D-161 — `VirtualThread`'s state machine

- **Target filename:** `D-161-virtualthread-s-state-machine.svg`
- **Type:** state-transition
- **Syllabus leaf:** 3.14.4
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** The nine states — `NEW`, `STARTED`, `RUNNABLE`, `RUNNING`, `PARKING`, `PARKED`, `PINNED`, `YIELDING`, `TERMINATED` — with every transition labelled by the event that causes it (`start`, schedule, mount, blocking call, successful yield, failed yield while pinned, unpark, completion)
