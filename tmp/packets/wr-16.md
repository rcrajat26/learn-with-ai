# Writer packet — `streams/06-cost-model.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/06-cost-model.md
```
...
| Field | Value |
|---|---|
| File | `streams/06-cost-model.md` |
| Subject | streams |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.3 — Streams: the cost model, and when not to use one |
| Leaves | 2.3.1–2.3.16 (16 leaves) |
| Primary concepts | what a pipeline costs against a loop; the allocation profile before the first element moves; debuggability and stack depth; ordering as optimisation; `sorted().findFirst()` versus `min`; when to use a loop and when a stream |
| Diagrams | D-096, D-097, D-098, D-169 |
| Examples (QuizStakes slice) | A three-stage pipeline over card deposits; comparator-invocation counts at N = 95,000; the accidental O(n·m) from re-streaming restrictions inside a loop over clients |
| Previous | `streams/05-primitive-streams.md` |
| Next | `streams/07-parallel-streams.md` |
...
- **D-096 — What exists before the first element moves** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-096 — What exists before the first element moves](../diagrams/D-096-exists-before-first-element.svg)
  Beneath each embed put the caption line `**D-096** — What exists before the first element moves`. The diagram shows: A three-stage pipeline over card deposits: the source spliterator, three `AbstractPipeline` stage objects doubly linked, three lambda instances (one non-capturing shared, two capturing), and the terminal op. Object count and approximate bytes totalled, then compared with a `for` loop's zero
- **D-097 — `sorted().findFirst()` versus `min(comparator)`** · type `cost-curve` · the file(s) already exist; embed exactly:
    ![D-097 — `sorted().findFirst()` versus `min(comparator)`](../diagrams/D-097-sorted-findfirst-versus-min.svg)
  Beneath each embed put the caption line `**D-097** — `sorted().findFirst()` versus `min(comparator)``. The diagram shows: Two curves over N from 10 to 2.8M: O(n log n) for sort-then-take-first and O(n) for `min`. Both plotted with the comparator-invocation counts at N = 95,000 (one day of card deposits) written on the curves. The identical answer noted
- **D-098 — Stream or loop** · type `decision-tree` · the file(s) already exist; embed exactly:
    ![D-098 — Stream or loop](../diagrams/D-098-stream-loop.svg)
  Beneath each embed put the caption line `**D-098** — Stream or loop`. The diagram shows: Root: "what does the code need to do?" Branches to loop for side effects, early exit carrying several values, index arithmetic, in-place mutation, checked exceptions, a measured hot path. Branches to stream for transformation chains, grouping and aggregation, laziness over an expensive or infinite source, one-line parallelism over a splittable source
- **D-169 — What a stream stack trace actually looks like** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-169 — What a stream stack trace actually looks like](../diagrams/D-169-stream-stack-trace-actually.svg)
  Beneath each embed put the caption line `**D-169** — What a stream stack trace actually looks like`. The diagram shows: Left: the exception stack trace from an NPE thrown inside a `map` over card deposits — the synthetic `lambda$process$2` frame, the `ReferencePipeline$3$1.accept` frames, the `AbstractPipeline.copyInto` frame, all labelled with what each one is. Right: the same failure inside a `for` loop, four frames deep. A note on the `StackOverflowError` risk from a long pipeline plus recursion plus `flatMap`
