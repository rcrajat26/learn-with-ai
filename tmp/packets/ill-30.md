# Illustrator packet ill-30 — topic 04 Modern Java
...
- **Target filename:** `D-154-record-deconstruction-accessor-calls.svg`
- **Type:** step-sequence, 3 frames
- **Syllabus leaf:** 3.11.8, 3.11.9
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** `case Movement(LedgerEntry(Position from, Money amount), LedgerEntry to)`. Frame 1: the outer type test. Frame 2: accessors invoked in declaration order, short-circuiting on the first component mismatch — the skipped calls marked. Frame 3: an accessor throwing, wrapped in `MatchException` with the original as its cause

### D-155 — `tableswitch` versus `lookupswitch`

- **Target filename:** `D-155-tableswitch-versus-lookupswitch.svg`
- **Type:** before-after
- **Syllabus leaf:** 3.12.1, 3.12.2
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Left: dense case labels compiling to `tableswitch` with the jump table drawn. Right: sparse labels compiling to `lookupswitch` with the key/offset pairs drawn. Beneath, the two-stage `String` switch: `lookupswitch` on `hashCode`, `equals` to confirm, then a second switch on a synthetic index — worked on a restriction-type name

### D-156 — `$SwitchMap` protects a separately compiled enum switch

- **Target filename:** `D-156-switchmap-protects-separately-compiled.svg`
- **Type:** before-after
- **Syllabus leaf:** 3.12.3
- **Must show (this is the contract — every named label, constant and value must be visible as text in the SVG):** Left: a switch over `RestrictionType` compiled with the synthetic `$SwitchMap$RestrictionType` `int[]` mapping `ordinal()` to a stable case index — the array contents shown. Right: the enum reordered and recompiled without recompiling the switch — the map absorbs the change and the correct arm still runs. The failure that would occur without it drawn as a third panel

### D-157 — The synthetic default in an exhaustive enum switch expression

- **Target filename:** `D-157-synthetic-default-exhaustive-enum.svg`
- **Type:** before-after
- **Syllabus leaf:** 3.12.7, 3.12.8
