# Writer packet — `platform-and-releases/03-internals-version-delta.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/platform-and-releases/03-internals-version-delta.md
```
...
| Field | Value |
|---|---|
| File | `platform-and-releases/03-internals-version-delta.md` |
| Subject | platform and releases |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.16 — Version-by-version delta |
| Leaves | 3.16.1–3.16.22 (22 leaves) |
| Primary concepts | the release-by-release delta 8 to 25; the consolidated feature-to-version table; the removed-or-disabled table; how to answer "what is new in Java N" |
| Diagrams | D-166, D-167 |
| Examples (QuizStakes slice) | Dating every claim in the guide against the release QuizStakes actually runs; §12 payment flows as the code being upgraded |
| Previous | `platform-and-releases/02-migration.md` |
| Next | `platform-and-releases/04-internals-observability.md` |
| Target size | **980 lines**, and anything from 686 to 1323 is fine |
...
- **D-166 — The consolidated feature → version table** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-166** — The consolidated feature → version table` on the line beneath it. It must contain every row, column and value named here: One row per feature this guide covers. Columns: first preview release, final release, JEP numbers for each stage, whether it is still preview on 21, and the one-line summary
- **D-167 — The consolidated removed-or-disabled table** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-167** — The consolidated removed-or-disabled table` on the line beneath it. It must contain every row, column and value named here: Rows: Nashorn, the Java EE modules, CORBA, applets, the Security Manager, finalization, the 32-bit x86 port, `sun.misc.Unsafe` memory access. Columns: deprecated in, removed or disabled in, the replacement, and the symptom on upgrade
...
```
path: platform-and-releases/03-internals-version-delta.md
lines: <wc -l of the file you wrote>
leaves: 3.16.1–3.16.22
