# Writer packet — `structured-concurrency/02-in-practice.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/structured-concurrency/02-in-practice.md
```
...
| Field | Value |
|---|---|
| File | `structured-concurrency/02-in-practice.md` |
| Subject | structured concurrency |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.13 — Structured concurrency and scoped values in practice |
| Leaves | 2.13.1–2.13.10 (10 leaves) |
| Primary concepts | the fan-out call with one deadline and one failure policy; hedged requests; timeouts at scope versus subtask level; which exception surfaces; nesting scopes; scoped values for request context; rebinding as shadowing; what to say in an interview |
| Diagrams | D-120, D-121 |
| Examples (QuizStakes slice) | A 2 s `joinUntil` deadline cutting off the watchlist provider; tenant, principal and trace id carried as scoped values instead of MDC `ThreadLocal`s |
| Previous | `structured-concurrency/01-basics.md` |
| Next | `structured-concurrency/03-internals.md` |
...
```
path: structured-concurrency/02-in-practice.md
lines: <wc -l of the file you wrote>
leaves: 2.13.1–2.13.10
