# Writer packet — `streams/08-internals-pipeline.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/08-internals-pipeline.md
```
...
| Field | Value |
|---|---|
| File | `streams/08-internals-pipeline.md` |
| Subject | streams |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.3 — Stream pipeline internals |
| Leaves | 3.3.1–3.3.20 (20 leaves) |
| Primary concepts | `AbstractPipeline`'s twelve fields and the stage chain; `Sink`'s four-method protocol; `opWrapSink` and `wrapSink` walking backwards; `copyInto`/`copyIntoWithCancel`; the `StreamOpFlag` lattice; how `count()` bypasses the pipeline |
| Diagrams | D-131, D-132, D-133, D-134 |
| Examples (QuizStakes slice) | `deposits.stream().filter(...).map(...).collect(...)` walked stage by stage with `depth` 0/1/2; the two `linkedOrConsumed` messages verbatim |
| Previous | `streams/07-parallel-streams.md` |
| Next | `streams/09-internals-spliterator.md` |
...
- **D-131 — The pipeline as a doubly linked list of stages** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-131 — The pipeline as a doubly linked list of stages](../diagrams/D-131-pipeline-doubly-linked-list.svg)
  Beneath each embed put the caption line `**D-131** — The pipeline as a doubly linked list of stages`. The diagram shows: The `AbstractPipeline` chain for `deposits.stream().filter(...).map(...).collect(...)`: three stage objects with all twelve fields named on the source stage (`sourceStage`, `previousStage`, `sourceOrOpFlags`, `nextStage`, `depth`, `combinedFlags`, `sourceSpliterator`, `sourceSupplier`, `linkedOrConsumed`, `sourceAnyStateful`, `sourceCloseAction`, `parallel`), `depth` values 0, 1, 2, and the `StatelessOp`/`StatefulOp` subtype labelled on each
- **D-132 — `wrapSink` walks backwards** · type `step-sequence, 4 frames` · the file(s) already exist; embed exactly:
    ![D-132 — `wrapSink` walks backwards](../diagrams/D-132-wrapsink-walks-backwards.svg)
  Beneath each embed put the caption line `**D-132** — `wrapSink` walks backwards`. The diagram shows: Frame 1: the terminal op's sink created. Frame 2: the `map` stage's `opWrapSink` wraps it. Frame 3: the `filter` stage wraps that. Frame 4: `copyInto` calls `begin`, `forEachRemaining`, `end` on the outermost sink, and one element traverses the whole chain. `Sink`'s four methods listed in a legend, and the `copyIntoWithCancel` variant noted on frame 4
- **D-133 — `StreamOpFlag`** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-133** — `StreamOpFlag`` on the line beneath it. It must contain every row, column and value named here: Rows: `DISTINCT`, `SORTED`, `ORDERED`, `SIZED`, `SHORT_CIRCUIT`. Columns: the bit position, what it means, what the stream position/op position/terminal-op position each encode (SET/CLEAR/PRESERVE), which operations set it, which clear it, and which optimisation it unlocks (`sorted()` becoming a no-op, `distinct()` using adjacent comparison, `count()` bypassing the pipeline)
- **D-134 — How `count()` bypasses the pipeline** · type `flowchart` · the file(s) already exist; embed exactly:
    ![D-134 — How `count()` bypasses the pipeline](../diagrams/D-134-count-bypasses-pipeline.svg)
  Beneath each embed put the caption line `**D-134** — How `count()` bypasses the pipeline`. The diagram shows: Start at `count()`. Check `SIZED` still set in `combinedFlags` → check no stateful op cleared it → check nothing short-circuits → return the source's exact size without building a sink chain. The `peek` stage boxed on the bypassed path and labelled "never invoked". A second path with a `filter` present, clearing `SIZED`, taking the full traversal. VERSION TRAP: Java 9 changed this
...
```
path: streams/08-internals-pipeline.md
