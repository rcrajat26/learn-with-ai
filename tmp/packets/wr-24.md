# Writer packet — `collectors/03-internals-collectors.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/collectors/03-internals-collectors.md
```
...
| Field | Value |
|---|---|
| File | `collectors/03-internals-collectors.md` |
| Subject | collectors |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.6 — Collector internals |
| Leaves | 3.6.1–3.6.10 (10 leaves) |
| Primary concepts | `CollectorImpl` and the six pre-built characteristic sets; `toList`'s three functions and the O(n) combine tree; `groupingBy`'s `computeIfAbsent` and its unchecked-cast finisher; Kahan compensation in `summingDouble`; what `IDENTITY_FINISH` saves |
| Diagrams | D-143, D-144, D-145 |
| Examples (QuizStakes slice) | Summing 95,000 card deposits averaging 65 as `double`s, naive total against compensated total |
| Previous | `collectors/02-in-anger.md` |
| Next | `optional/01-basics.md` |
| Target size | **844 lines**, and anything from 590 to 1139 is fine |
...
- **D-143 — `CollectorImpl` and its pre-built characteristic sets** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-143** — `CollectorImpl` and its pre-built characteristic sets` on the line beneath it. It must contain every row, column and value named here: Rows: `CH_CONCURRENT_ID`, `CH_CONCURRENT_NOID`, `CH_ID`, `CH_UNORDERED_ID`, `CH_UNORDERED_NOID`, `CH_NOID`. Columns: which of `CONCURRENT`/`UNORDERED`/`IDENTITY_FINISH` each contains, which collectors use it, and what the framework does differently as a result
- **D-144 — Kahan compensated summation inside `summingDouble`** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-144 — Kahan compensated summation inside `summingDouble`](../diagrams/D-144-kahan-compensated-summation-inside.svg)
  Beneath each embed put the caption line `**D-144** — Kahan compensated summation inside `summingDouble``. The diagram shows: Summing 95,000 card deposits averaging 65 as `double`s. Frame 1: the three-element `double[]` accumulator with the running sum, the compensation term and the simple sum. Frame 2: a small addend lost to a naive `+`, recovered into the compensation slot — the arithmetic written. Frame 3: the compensation added back in the finisher, with the naive total and the compensated total printed side by side. A note that `summingInt` uses a `long[]` and needs none of this
- **D-145 — `IDENTITY_FINISH` skips a whole pass** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-145 — `IDENTITY_FINISH` skips a whole pass](../diagrams/D-145-identity-finish-skips-whole.svg)
  Beneath each embed put the caption line `**D-145** — `IDENTITY_FINISH` skips a whole pass`. The diagram shows: Left, a collector without `IDENTITY_FINISH`: the accumulation container built, then the finisher walks it to produce the result — two passes drawn. Right, with `IDENTITY_FINISH`: the container is cast and returned directly, one pass. The saved work stated in element counts over 95,000 deposits
...
```
path: collectors/03-internals-collectors.md
