# Writer packet — `virtual-threads/03-internals-virtual-threads.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/virtual-threads/03-internals-virtual-threads.md
```
...
| Field | Value |
|---|---|
| File | `virtual-threads/03-internals-virtual-threads.md` |
| Subject | virtual threads |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.14 — Virtual thread internals |
| Leaves | 3.14.1–3.14.18 (18 leaves) |
| Primary concepts | the three layers and `Continuation`; frame copying to and from a heap `StackChunk`; the nine-state machine; the FIFO scheduler and its verified defaults; the instrumented and non-instrumented blocking points; pinning as a continuation property and JEP 491; no preemption and pool compensation |
| Diagrams | D-159, D-160, D-161, D-162, D-163 |
| Examples (QuizStakes slice) | A virtual thread blocking on the card PSP's 240 ms p50 across four mount/unmount frames; the heap arithmetic for 1,000,000 virtual threads |
| Previous | `virtual-threads/02-in-production.md` |
| Next | `structured-concurrency/01-basics.md` |
...
- **D-159 — The three layers of a virtual thread** · type `hierarchy` · the file(s) already exist; embed exactly:
    ![D-159 — The three layers of a virtual thread](../diagrams/D-159-three-layers-virtual-thread.svg)
  Beneath each embed put the caption line `**D-159** — The three layers of a virtual thread`. The diagram shows: `java.lang.VirtualThread` on top, `jdk.internal.vm.Continuation` beneath it, the FIFO `ForkJoinPool` scheduler beneath that, and platform carrier threads at the bottom. `Thread.currentThread()` drawn returning the `VirtualThread`, with the carrier marked reachable only through internal API
- **D-160 — Stack chunks live on the heap** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-160 — Stack chunks live on the heap](../diagrams/D-160-stack-chunks-live-heap.svg)
  Beneath each embed put the caption line `**D-160** — Stack chunks live on the heap`. The diagram shows: One carrier's native stack with three mounted frames; the heap holding `StackChunk` objects for the unmounted threads; arrows for the copy in both directions labelled "mount" and "unmount", with the lazy/partial copying noted. The heap arithmetic for 1,000,000 virtual threads written out and contrasted with 1,000,000 × 1 MB of reserved platform stack
- **D-161 — `VirtualThread`'s state machine** · type `state-transition` · the file(s) already exist; embed exactly:
    ![D-161 — `VirtualThread`'s state machine](../diagrams/D-161-virtualthread-s-state-machine.svg)
  Beneath each embed put the caption line `**D-161** — `VirtualThread`'s state machine`. The diagram shows: The nine states — `NEW`, `STARTED`, `RUNNABLE`, `RUNNING`, `PARKING`, `PARKED`, `PINNED`, `YIELDING`, `TERMINATED` — with every transition labelled by the event that causes it (`start`, schedule, mount, blocking call, successful yield, failed yield while pinned, unpark, completion)
- **D-162 — FIFO for virtual threads, LIFO for parallel streams** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-162 — FIFO for virtual threads, LIFO for parallel streams](../diagrams/D-162-fifo-virtual-threads-lifo.svg)
  Beneath each embed put the caption line `**D-162** — FIFO for virtual threads, LIFO for parallel streams`. The diagram shows: Left: the virtual-thread scheduler in FIFO async mode — independent tasks, fairness prioritised, the queue drawn head-first. Right: the common pool's LIFO work-stealing — recursively split subtasks, locality prioritised, own-head push/pop with tail stealing. The reason for each choice written under each panel
- **D-163 — Pinning is a property of the continuation** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-163 — Pinning is a property of the continuation](../diagrams/D-163-pinning-property-continuation.svg)
  Beneath each embed put the caption line `**D-163** — Pinning is a property of the continuation`. The diagram shows: Left, Java 21: a held monitor or a native frame on the continuation's stack makes `yield` impossible — both frames marked on a drawn stack. Right, Java 24 (JEP 491): monitors are continuation-aware and the monitor case disappears; the native frame still pins. A third panel: no preemption — a CPU-bound virtual thread holding its carrier indefinitely, and the carrier pool growing toward `maxPoolSize` as compensation
...
```
path: virtual-threads/03-internals-virtual-threads.md
lines: <wc -l of the file you wrote>
leaves: 3.14.1–3.14.18
