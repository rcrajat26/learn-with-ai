# Writer packet — `collectors/01-basics-b.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/collectors/01-basics-b.md
```
...
| Field | Value |
|---|---|
| File | `collectors/01-basics-b.md` |
| Subject | collectors |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.10 — Collectors |
| Leaves | 1.10.17–1.10.30 (14 leaves) |
| Primary concepts | `groupingBy`'s three overloads and the types it really returns; the null-classifier NPE; `partitioningBy` always carrying both keys; `groupingByConcurrent` and the three conditions for a concurrent reduction; `teeing`; hand-writing a collector; the collector inventory |
| Diagrams | D-041, D-042, D-043, D-044 |
| Examples (QuizStakes slice) | `groupingBy(Deposit::rail, mapping(Deposit::amount, toList()))`; `partitioningBy` over an empty reservation stream; `teeing` for min-and-max withdrawal in one pass |
| Previous | `collectors/01-basics-a.md` |
| Next | `collectors/02-in-anger.md` |
...
- **D-041 — What `groupingBy` actually returns** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-041 — What `groupingBy` actually returns](../diagrams/D-041-groupingby-actually-returns.svg)
  Beneath each embed put the caption line `**D-041** — What `groupingBy` actually returns`. The diagram shows: The result of `groupingBy(Deposit::rail, mapping(Deposit::amount, toList()))` over card and bank deposits: a `HashMap` with its bucket table, two entries, each value an `ArrayList`. Both concrete types labelled "not guaranteed by the contract", with `TreeMap::new` and `LinkedHashMap::new` drawn as the ordered alternatives
- **D-042 — `partitioningBy` always has both keys** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-042 — `partitioningBy` always has both keys](../diagrams/D-042-partitioningby-always-has-both.svg)
  Beneath each embed put the caption line `**D-042** — `partitioningBy` always has both keys`. The diagram shows: Over an empty stream of reservations. Left, `groupingBy(r -> r.amount().compareTo(MAX) > 0)`: an empty map, `get(true)` returns null. Right, `partitioningBy(...)`: a two-entry map with `false → []` and `true → []`. The NPE the left produces on unboxing drawn as the consequence
- **D-043 — The three conditions for a concurrent reduction** · type `decision-tree` · the file(s) already exist; embed exactly:
    ![D-043 — The three conditions for a concurrent reduction](../diagrams/D-043-three-conditions-concurrent-reduction.svg)
  Beneath each embed put the caption line `**D-043** — The three conditions for a concurrent reduction`. The diagram shows: Root: `collect` called. Node 1: is the stream parallel? Node 2: is the collector `CONCURRENT`? Node 3: is the stream unordered *or* the collector `UNORDERED`? All three yes → one shared container, no combiner. Any no → per-leaf containers and a combiner tree. Both outcomes drawn with their container counts
- **D-044 — Why `collect(toList())` is safe in parallel and `forEach(list::add)` is not** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-044 — Why `collect(toList())` is safe in parallel and `forEach(list::add)` is not](../diagrams/D-044-collect-tolist-safe-parallel.svg)
  Beneath each embed put the caption line `**D-044** — Why `collect(toList())` is safe in parallel and `forEach(list::add)` is not`. The diagram shows: Left: four leaves each with their own `ArrayList`, merged pairwise up a combiner tree — no shared state, arrows labelled with sizes. Right: four leaves all calling `add` on one `ArrayList`, with the three observable symptoms named: lost elements, interspersed nulls, and `ArrayIndexOutOfBoundsException` from inside `ArrayList.add`
...
```
path: collectors/01-basics-b.md
lines: <wc -l of the file you wrote>
leaves: 1.10.17–1.10.30
