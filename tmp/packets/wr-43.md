# Writer packet — `text-blocks/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/text-blocks/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `text-blocks/01-basics.md` |
| Subject | text blocks |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.17 — Text blocks |
| Leaves | 1.17.1–1.17.16 (16 leaves) |
| Primary concepts | the syntax and the opening-delimiter rule; the three compile-time steps in order; incidental-whitespace computation including the closing delimiter; trailing-whitespace stripping; `\s` and `\` line continuation; the runtime siblings; text blocks as constant expressions |
| Diagrams | D-069, D-070, D-071, D-072 |
| Examples (QuizStakes slice) | The SQL text block that reads `CLIENT_CASH_AVAILABLE` positions from the ledger, with the closing delimiter moved four columns left |
| Previous | `switch/03-internals-switch-compilation.md` |
| Next | `text-blocks/02-in-practice.md` |
| Target size | **924 lines**, and anything from 646 to 1247 is fine |
...
These are the **16 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- **D-069 — The three text-block compile steps, in order** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-069 — The three text-block compile steps, in order](../diagrams/D-069-three-text-block-compile.svg)
  Beneath each embed put the caption line `**D-069** — The three text-block compile steps, in order`. The diagram shows: A SQL text block reading `CLIENT_CASH_AVAILABLE` positions. Frame 1: CRLF line terminators normalised to `\n`, both forms shown as visible characters. Frame 2: incidental whitespace removed. Frame 3: escape sequences translated, with a `\n` the author wrote surviving frame 1 untouched and a `\s` surviving frame 2. Order labelled "and no other order"
- **D-070 — How incidental whitespace is computed** · type `step-sequence, 4 frames` · the file(s) already exist; embed exactly:
    ![D-070 — How incidental whitespace is computed](../diagrams/D-070-incidental-whitespace-computed.svg)
  Beneath each embed put the caption line `**D-070** — How incidental whitespace is computed`. The diagram shows: A four-line JSON fixture. Frame 1: trailing whitespace stripped from every line, the stripped characters marked. Frame 2: blank lines excluded from the minimum. Frame 3: the closing delimiter's line *included* in the minimum, its indentation marked. Frame 4: the common prefix removed, the result shown with a left margin ruler and column numbers
- **D-071 — Moving the closing delimiter changes the string** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-071 — Moving the closing delimiter changes the string](../diagrams/D-071-moving-closing-delimiter-changes.svg)
  Beneath each embed put the caption line `**D-071** — Moving the closing delimiter changes the string`. The diagram shows: The same SQL text block drawn twice with a column ruler: closing `"""` aligned with the content, and closing `"""` four columns to the left. The two resulting strings printed with a visible left-margin marker per line and the extra four spaces highlighted
- **D-072 — `\s` as a trailing-space fence** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-072 — `\s` as a trailing-space fence](../diagrams/D-072-s-trailing-space-fence.svg)
  Beneath each embed put the caption line `**D-072** — `\s` as a trailing-space fence`. The diagram shows: A fixed-width payload line whose trailing spaces are significant. Left: without `\s` — the trailing spaces stripped, the field width wrong, both lengths printed. Right: with `\s` at the end — the space survives, the width correct. A second row shows `\` at end of line suppressing the terminator
...
```
path: text-blocks/01-basics.md
lines: <wc -l of the file you wrote>
leaves: 1.17.1–1.17.16
