# Writer packet — `records/02-in-practice.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/records/02-in-practice.md
```
...
| Field | Value |
|---|---|
| File | `records/02-in-practice.md` |
| Subject | records |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.8 — Records in practice |
| Leaves | 2.8.1–2.8.16 (16 leaves) |
| Primary concepts | records as DTOs at an HTTP boundary; Jackson and Spring binding and the `-parameters` flag; Bean Validation targets; why a record cannot be a JPA entity but is an excellent projection; compound map keys; local records; the wither pattern; floating-point components |
| Diagrams | D-108, D-109 |
| Examples (QuizStakes slice) | A `DepositRequest`/`DepositResponse` pair at the `ApplicationGateway`; `RestrictionKey(RestrictionType, RestrictionSource)` as a compound map key |
| Previous | `records/01-basics-b.md` |
| Next | `records/03-internals-records.md` |
...
- **D-108 — Records across the framework boundary** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-108** — Records across the framework boundary` on the line beneath it. It must contain every row, column and value named here: Rows: Jackson serialisation, Jackson deserialisation, Spring `@RequestBody`, Spring `@ConfigurationProperties`, Spring `@ModelAttribute`, Bean Validation, JPA entity, JPA `@Embeddable`, Spring Data projection, Lombok `@Value` equivalence. Columns: works, minimum version, what it needs (`-parameters`, `@JsonProperty`, `@JsonCreator`, a `@Target` including `RECORD_COMPONENT`), and the failure symptom when it does not
- **D-109 — Defensive copying, in and out** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-109 — Defensive copying, in and out](../diagrams/D-109-defensive-copying-out.svg)
  Beneath each embed put the caption line `**D-109** — Defensive copying, in and out`. The diagram shows: `record PaymentRun(RunId id, List<WithdrawalTransaction> items, byte[] signature)`. Left: no copies — the caller's list and array both shared and mutable through the accessor. Right: `List.copyOf` in the compact constructor and `signature.clone()` on both copy-in and copy-out, with four distinct objects drawn and the mutation attempts shown failing
...
```
path: records/02-in-practice.md
