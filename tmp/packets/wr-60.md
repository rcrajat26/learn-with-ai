# Writer packet — `build-it/06-filling-the-21-gaps.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/build-it/06-filling-the-21-gaps.md
```
...
| Field | Value |
|---|---|
| File | `build-it/06-filling-the-21-gaps.md` |
| Subject | build it |
| Part / tier | Part 4 — BUILD IT |
| Syllabus sections | §4.7 — Filling the Java 21 gaps |
| Leaves | 4.7.1–4.7.6 (6 leaves) |
| Primary concepts | fixed-window batching via a custom `Spliterator`; `zip` via a paired spliterator; `scan` and `distinctBy` as stateful mappers with their parallel failure demonstrated; `takeUntil` and a `mapConcurrent` on virtual threads; the `Gatherers` diff |
| Diagrams | D-177 |
| Examples (QuizStakes slice) | Fixed windows of 100 ledger entries out of the ~19.8M written per day |
| Previous | `build-it/05-concurrency-builds.md` |
| Next | `build-it/07-diagnostic-harnesses.md` |
| Target size | **780 lines**, and anything from 546 to 1053 is fine |
...
These are the **6 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- **D-177 — Hand-rolled batching versus `Gatherers.windowFixed`** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-177 — Hand-rolled batching versus `Gatherers.windowFixed`](../diagrams/D-177-hand-rolled-batching-versus.svg)
  Beneath each embed put the caption line `**D-177** — Hand-rolled batching versus `Gatherers.windowFixed``. The diagram shows: Left: the Java 21 custom `Spliterator` for fixed windows of 100 ledger entries, with `estimateSize` and the absent `SUBSIZED` claim labelled. Right: the Java 24 `Gatherers.windowFixed(100)` one-liner, with the `Gatherer` contract (`initializer`, `integrator`, `combiner`, `finisher`) and greedy-vs-short-circuiting integrators listed
...
```
path: build-it/06-filling-the-21-gaps.md
