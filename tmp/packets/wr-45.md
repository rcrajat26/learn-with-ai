# Writer packet — `text-blocks/03-internals-compilation.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/text-blocks/03-internals-compilation.md
```
...
| Field | Value |
|---|---|
| File | `text-blocks/03-internals-compilation.md` |
| Subject | text blocks |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.13 — Text block compilation |
| Leaves | 3.13.1–3.13.6 (6 leaves) |
| Primary concepts | the whole transformation happening in `javac`; the specified three-step algorithm; the exact minimal-indent computation; the result as a `CONSTANT_String_info` and therefore interned; `String.stripIndent()` as the runtime sibling; `==` on a text block and an equal literal |
| Diagrams | D-158 |
| Examples (QuizStakes slice) | The `javap -v` constant pool for the ledger SQL text block, already stripped |
| Previous | `text-blocks/02-in-practice.md` |
| Next | `virtual-threads/01-basics.md` |
...
- **D-158 — A text block is a constant, folded at compile time** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-158 — A text block is a constant, folded at compile time](../diagrams/D-158-text-block-constant-folded.svg)
  Beneath each embed put the caption line `**D-158** — A text block is a constant, folded at compile time`. The diagram shows: Left: the source text block. Right: the `javap -v` constant pool showing a single `CONSTANT_String_info` with the final, already-stripped content — nothing of the algorithm surviving to runtime. Beneath: a text block and an equal string literal compared with `==`, both pointing at the same interned constant, with the caution that this is not a habit to build
...
```
path: text-blocks/03-internals-compilation.md
lines: <wc -l of the file you wrote>
leaves: 3.13.1–3.13.6
