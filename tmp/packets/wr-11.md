# Writer packet — `streams/01-basics-the-model.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/01-basics-the-model.md
```
...
| Field | Value |
|---|---|
| File | `streams/01-basics-the-model.md` |
| Subject | streams |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.5 — The stream model |
| Leaves | 1.5.1–1.5.18 (18 leaves) |
| Primary concepts | the javadoc's five properties; source/intermediate/terminal anatomy; laziness and fusion; short-circuiting; encounter order; non-interference and statelessness; single consumption; closing |
| Diagrams | D-018, D-019, D-020, D-021, D-022 |
| Examples (QuizStakes slice) | A pipeline over 95k card deposits per day; the two exact `IllegalStateException` messages; `Files.lines(paymentRunFile)` needing a close |
| Previous | `method-references/01-basics.md` |
| Next | `streams/02-sources.md` |
| Target size | **956 lines**, and anything from 669 to 1290 is fine |
...
These are the **18 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- **D-018 — Stream anatomy: source, intermediates, terminal** · type `step-sequence, 3 frames` · authored as **3 frame files**, and you must embed **all of them, in order**, each with its own caption line naming the frame · the file(s) already exist; embed exactly:
  ...
  Beneath each embed put the caption line `**D-018** — Stream anatomy: source, intermediates, terminal`. The diagram shows: A pipeline over 95k card deposits: `deposits.stream().filter(...).map(...).collect(toList())`. Frame 1: the source `Spliterator` bound. Frame 2: two stage objects created, nothing traversed, "0 elements have moved" labelled. Frame 3: the terminal operation triggers traversal. Each frame lists which objects exist so far
- **D-019 — Fusion: one element through the whole chain** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-019 — Fusion: one element through the whole chain](../diagrams/D-019-fusion-element-through-whole.svg)
  Beneath each embed put the caption line `**D-019** — Fusion: one element through the whole chain`. The diagram shows: Left, the wrong mental model: three passes over the whole collection with two intermediate lists materialised, sizes labelled. Right, the real model: one element at a time entering `filter`, then `map`, then the collector, with a numbered trace for the first three deposits (65, 480, 65) and no intermediate collection anywhere
- **D-020 — Laziness, statefulness and short-circuiting, per operation** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-020** — Laziness, statefulness and short-circuiting, per operation` on the line beneath it. It must contain every row, column and value named here: One row per intermediate operation. Columns: lazy (always yes), stateless/stateful, short-circuiting, buffering required, encounter-order sensitive. A second block does terminal operations with eager/lazy and short-circuiting
- **D-021 — A stream is consumed once** · type `state-transition` · the file(s) already exist; embed exactly:
    ![D-021 — A stream is consumed once](../diagrams/D-021-stream-consumed-once.svg)
  Beneath each embed put the caption line `**D-021** — A stream is consumed once`. The diagram shows: Three states — unconsumed, linked (an intermediate op attached), consumed/closed — with the transitions labelled by the call that causes them and the two exact exception messages on the illegal edges: `"stream has already been operated upon or closed"` and `"source already consumed or closed"`
- **D-022 — Which streams must be closed** · type `decision-tree` · the file(s) already exist; embed exactly:
    ![D-022 — Which streams must be closed](../diagrams/D-022-streams-must-closed.svg)
  Beneath each embed put the caption line `**D-022** — Which streams must be closed`. The diagram shows: Root: "does the source hold an OS resource?" Yes branch lists `Files.lines`, `Files.walk`, `Files.list`, `Files.find`, `Files.newDirectoryStream` → try-with-resources required, with the file-descriptor leak as the symptom. No branch lists `Collection.stream`, `Arrays.stream`, `IntStream.range` → closing is a no-op. `onClose(Runnable)` shown on the yes branch
...
```
path: streams/01-basics-the-model.md
