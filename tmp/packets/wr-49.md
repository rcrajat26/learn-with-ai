# Writer packet — `structured-concurrency/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/structured-concurrency/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `structured-concurrency/01-basics.md` |
| Subject | structured concurrency |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.19 — Structured concurrency |
| Leaves | 1.19.1–1.19.16 (16 leaves) |
| Primary concepts | the leak/cancellation/dump problem; the structured principle; the Java 21 `StructuredTaskScope` shape with `Subtask`; `ShutdownOnFailure` and `ShutdownOnSuccess`; `joinUntil`; the ownership and try-with-resources discipline; cancellation by interrupt; the comparison with `allOf` and `invokeAll`; scoped values |
| Diagrams | D-080, D-081, D-082, D-083 |
| Examples (QuizStakes slice) | `AssessmentService` forking the identity vendor (900 ms p50) and the watchlist provider (1.4 s p50, 25 s p99) under one scope |
| Previous | `virtual-threads/03-internals-virtual-threads.md` |
| Next | `structured-concurrency/02-in-practice.md` |
...
- **D-080 — A structured task scope is a tree** · type `hierarchy` · the file(s) already exist; embed exactly:
    ![D-080 — A structured task scope is a tree](../diagrams/D-080-structured-task-scope-tree.svg)
  Beneath each embed put the caption line `**D-080** — A structured task scope is a tree`. The diagram shows: An `AssessmentService` call forking two subtasks — the identity vendor (p50 900 ms) and the watchlist provider (p50 1.4 s) — under one scope, each subtask a virtual thread. The `try`-with-resources block drawn as the boundary the subtasks cannot outlive. Beside it the unstructured version with two orphan threads escaping the block
- **D-081 — `ShutdownOnFailure` versus `CompletableFuture.allOf`** · type `timeline` · the file(s) already exist; embed exactly:
  ...
  Beneath each embed put the caption line `**D-081** — `ShutdownOnFailure` versus `CompletableFuture.allOf``. The diagram shows: Two lanes on one time axis. Lane 1, `ShutdownOnFailure`: the watchlist call fails at 1.4 s, the identity call is interrupted, `join()` returns, `throwIfFailed()` rethrows. Lane 2, `allOf`: the same failure, the identity call keeps running past the block's end, marked "orphan", still holding its connection. Both end states labelled
- **D-082 — `ShutdownOnSuccess` as a hedge** · type `timeline` · the file(s) already exist; embed exactly:
    ![D-082 — `ShutdownOnSuccess` as a hedge](../diagrams/D-082-shutdownonsuccess-hedge.svg)
  Beneath each embed put the caption line `**D-082** — `ShutdownOnSuccess` as a hedge`. The diagram shows: Two replicas of the watchlist provider, one responding at 1.4 s and one at 25 s. One axis, two lanes: the fast one completes, the scope cancels the slow one at 1.4 s, the total latency marked. A comparison line shows the un-hedged p99 of 25 s
- **D-083 — `Subtask` states and the illegal calls** · type `state-transition` · the file(s) already exist; embed exactly:
    ![D-083 — `Subtask` states and the illegal calls](../diagrams/D-083-subtask-states-illegal-calls.svg)
  Beneath each embed put the caption line `**D-083** — `Subtask` states and the illegal calls`. The diagram shows: States `UNAVAILABLE`, `SUCCESS`, `FAILED`, with the transitions caused by `fork`, task completion, task failure and `shutdown`. Illegal edges labelled with their exceptions: `get()` before `join()` → `IllegalStateException`; a call from a non-owner thread or an out-of-order close → `StructureViolationException`
...
```
path: structured-concurrency/01-basics.md
lines: <wc -l of the file you wrote>
leaves: 1.19.1–1.19.16
