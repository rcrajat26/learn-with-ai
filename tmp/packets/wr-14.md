# Writer packet — `streams/04-terminal-operations.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/04-terminal-operations.md
```
...
| Field | Value |
|---|---|
| File | `streams/04-terminal-operations.md` |
| Subject | streams |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.8 — Terminal operations, exhaustively |
| Leaves | 1.8.1–1.8.26 (26 leaves) |
| Primary concepts | the three `reduce` overloads and their contracts; identity and associativity in parallel; `collect` versus `reduce` versus `forEach`; `count()`'s Java 9 bypass; vacuous `allMatch`; `findFirst` versus `findAny`; the null policy across the list-producing paths |
| Diagrams | D-031, D-032, D-033, D-034, D-035 |
| Examples (QuizStakes slice) | Summing `Money` over 95k card deposits; subtraction over `[65, 480, 42, 180]`; a four-leaf task tree over 2.8M reservations |
| Previous | `streams/03-intermediate-operations.md` |
| Next | `streams/05-primitive-streams.md` |
| Target size | **1052 lines**, and anything from 736 to 1420 is fine |
...
These are the **26 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- **D-031 — Terminal operation inventory** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-031** — Terminal operation inventory` on the line beneath it. It must contain every row, column and value named here: One row per terminal operation. Columns: version added, return type, eager or lazy, short-circuiting, parallel-friendly, ordering-sensitive, returns `Optional` and why
- **D-032 — The three `reduce` overloads** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-032** — The three `reduce` overloads` on the line beneath it. It must contain every row, column and value named here: Rows: `reduce(BinaryOperator)`, `reduce(identity, BinaryOperator)`, `reduce(identity, accumulator, combiner)`. Columns: return type, what the empty stream yields, the contracts you must satisfy (identity, associativity, compatibility of accumulator and combiner), and the QuizStakes example (summing `Money` over a day's 95k deposits)
- **D-033 — What a non-associative reduce does in parallel** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-033 — What a non-associative reduce does in parallel](../diagrams/D-033-non-associative-reduce-parallel.svg)
  Beneath each embed put the caption line `**D-033** — What a non-associative reduce does in parallel`. The diagram shows: Subtraction over `[65, 480, 42, 180]`. Frame 1: the sequential left fold with its result. Frame 2: the same operator split across two leaves and combined, with the different result written out. Frame 3: the same argument for string concatenation with a non-identity seed. Both wrong answers and the correct sequential answers labelled
- **D-034 — `findFirst` versus `findAny` in parallel** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-034 — `findFirst` versus `findAny` in parallel](../diagrams/D-034-findfirst-versus-findany-parallel.svg)
  Beneath each embed put the caption line `**D-034** — `findFirst` versus `findAny` in parallel`. The diagram shows: A four-leaf task tree over 2.8M reservations. Left, `findAny`: the first leaf to succeed wins and the rest cancel — no coordination arrows. Right, `findFirst`: leaves must report in encounter order, so later leaves' results are held until earlier leaves resolve — the coordination arrows drawn and labelled with the cost
- **D-035 — Null policy across the list-producing paths** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-035** — Null policy across the list-producing paths` on the line beneath it. It must contain every row, column and value named here: Rows: `Stream` elements, `Stream.toList()`, `Collectors.toList()`, `Collectors.toUnmodifiableList()`, `Collectors.toMap` key, `Collectors.toMap` value, `List.of`, `List.copyOf`, `Arrays.asList`, `new ArrayList<>()`. Columns: nulls permitted, mutable, structurally modifiable, `set` in place, exception thrown on violation
...
```
path: streams/04-terminal-operations.md
