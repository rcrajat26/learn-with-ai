# Writer packet — `structured-concurrency/03-internals.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/structured-concurrency/03-internals.md
```
...
| Field | Value |
|---|---|
| File | `structured-concurrency/03-internals.md` |
| Subject | structured concurrency |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.15 — Structured concurrency and scoped values internals |
| Leaves | 3.15.1–3.15.8 (8 leaves) |
| Primary concepts | `StructuredTaskScope` on virtual threads plus a per-thread scope stack; the ownership check; `StructureViolationException` and the stack discipline; `shutdown()` versus `close()`; `ScopedValue`'s immutable binding snapshot and its cache; why it is cheaper than `ThreadLocal`; the 19-to-26 churn table |
| Diagrams | D-164, D-165 |
| Examples (QuizStakes slice) | The `AssessmentService` scope tree in a JSON thread dump; a nested `where` shadowing a tenant binding |
| Previous | `structured-concurrency/02-in-practice.md` |
| Next | `library-additions/01-basics.md` |
| Target size | **812 lines**, and anything from 568 to 1096 is fine |
...
```
path: structured-concurrency/03-internals.md
lines: <wc -l of the file you wrote>
leaves: 3.15.1–3.15.8
