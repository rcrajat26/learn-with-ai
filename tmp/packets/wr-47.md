# Writer packet — `virtual-threads/02-in-production.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/virtual-threads/02-in-production.md
```
...
| Field | Value |
|---|---|
| File | `virtual-threads/02-in-production.md` |
| Subject | virtual threads |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.12 — Virtual threads in production |
| Leaves | 2.12.1–2.12.18 (18 leaves) |
| Primary concepts | the thread-per-request model restored and what the Spring flag switches; losing the pool means losing the queue; the bottleneck moving downstream; pinning drivers; `ThreadLocal` and MDC costs; thread dumps and the four JFR events; what to measure now; memory sizing; the migration checklist |
| Diagrams | D-116, D-117, D-118, D-119 |
| Examples (QuizStakes slice) | Tomcat at `maxThreads=200` against 55k peak sessions; 14k concurrent virtual threads arriving at a 20-connection JDBC pool |
| Previous | `virtual-threads/01-basics.md` |
| Next | `virtual-threads/03-internals-virtual-threads.md` |
...
- **D-116 — A Spring Boot request path, before and after virtual threads** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-116 — A Spring Boot request path, before and after virtual threads](../diagrams/D-116-spring-boot-request-path.svg)
  Beneath each embed put the caption line `**D-116** — A Spring Boot request path, before and after virtual threads`. The diagram shows: Left: Tomcat with `maxThreads=200` — 200 platform threads, requests queueing behind them, `maxThreads` acting as the accidental rate limiter, 55k peak sessions against a 200-wide gate. Right: `spring.threads.virtual.enabled=true` — a virtual thread per request, no concurrency cap at the container, and the queue gone. What the flag switches (servlet executor, `@Async`) and what it does not, labelled
- **D-117 — The bottleneck moves downstream** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-117 — The bottleneck moves downstream](../diagrams/D-117-bottleneck-moves-downstream.svg)
  Beneath each embed put the caption line `**D-117** — The bottleneck moves downstream`. The diagram shows: Left: the pool as the implicit limiter, with the JDBC pool of 20 comfortably behind 200 request threads. Right: 14k concurrent virtual threads arriving at the same 20-connection pool, the queue now at the connection pool and the database's max-connections ceiling. The deliberate fix — a `Semaphore` sized on purpose — drawn as the third panel
- **D-118 — A pinning JDBC driver under load** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-118 — A pinning JDBC driver under load](../diagrams/D-118-pinning-jdbc-driver-under.svg)
  Beneath each embed put the caption line `**D-118** — A pinning JDBC driver under load`. The diagram shows: Frame 1: eight carriers, each running a virtual thread that enters the driver's `synchronized` block and blocks on the network. Frame 2: all carriers pinned; new virtual threads cannot run; the JFR `jdk.VirtualThreadPinned` event fires past its 20 ms threshold. Frame 3: the JSON thread dump from `jcmd <pid> Thread.dump_to_file -format=json` showing the pinned frames, with a note that `jstack` shows none of it
- **D-119 — What to measure once threads are free** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-119** — What to measure once threads are free` on the line beneath it. It must contain every row, column and value named here: Rows: live threads gauge, in-flight requests, semaphore permits in use, connection-pool saturation, `jdk.VirtualThreadStart`/`End`, `jdk.VirtualThreadPinned`, `jdk.VirtualThreadSubmitFailed`, heap occupied by stack chunks. Columns: what it meant before, what it means now, default JFR state (enabled/disabled) and threshold, and the alert worth setting
...
```
path: virtual-threads/02-in-production.md
lines: <wc -l of the file you wrote>
leaves: 2.12.1–2.12.18
