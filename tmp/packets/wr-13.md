# Writer packet — `streams/03-intermediate-operations.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/03-intermediate-operations.md
```
...
| Field | Value |
|---|---|
| File | `streams/03-intermediate-operations.md` |
| Subject | streams |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.7 — Intermediate operations, exhaustively |
| Leaves | 1.7.1–1.7.24 (24 leaves) |
| Primary concepts | every intermediate operation with its flags; `flatMap` versus `mapMulti`; `takeWhile` as a prefix not a test; `sorted` as a barrier that throws at terminal time; `peek` elision; the absent `zip`/windowing; operation order as cost |
| Diagrams | D-025, D-026, D-027, D-028, D-029, D-030 |
| Examples (QuizStakes slice) | Stake amounts `[4.20, 3.33, 12.00, 2.10, 1.05]` under `amount < 5`; `.sorted(byAmount).limit(10)` over 2.8M stake reservations; `Movement` values holding zero, one or three `LedgerEntry`s |
| Previous | `streams/02-sources.md` |
| Next | `streams/04-terminal-operations.md` |
...
- **D-025 — Intermediate operation inventory** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-025** — Intermediate operation inventory` on the line beneath it. It must contain every row, column and value named here: One row per intermediate operation (`filter`, `map`, the four `mapToX`, `boxed`, `flatMap`, the three `flatMapToX`, `mapMulti` and its three primitive forms, `distinct`, `sorted` ×2, `limit`, `skip`, `takeWhile`, `dropWhile`, `peek`, `parallel`, `sequential`, `unordered`, `onClose`). Columns: version added, stateful, short-circuiting, and its effect (SET/CLEAR/PRESERVE) on each of `SIZED`, `ORDERED`, `DISTINCT`, `SORTED`
- **D-026 — `map` vs `flatMap` vs `mapMulti`** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-026 — `map` vs `flatMap` vs `mapMulti`](../diagrams/D-026-map-vs-flatmap-vs.svg)
  Beneath each embed put the caption line `**D-026** — `map` vs `flatMap` vs `mapMulti``. The diagram shows: The same input of three `Movement`s each holding zero, one or three `LedgerEntry` values. Frame 1: `map` produces three `List`s — cardinality 1:1. Frame 2: `flatMap` allocates one inner `Stream` per element and flattens — the three allocations drawn explicitly. Frame 3: `mapMulti` pushes into a `Consumer` with zero allocations — the same output. Allocation counts written on each frame
- **D-027 — `takeWhile` is a prefix, `filter` is a test** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-027 — `takeWhile` is a prefix, `filter` is a test](../diagrams/D-027-takewhile-prefix-filter-test.svg)
  Beneath each embed put the caption line `**D-027** — `takeWhile` is a prefix, `filter` is a test`. The diagram shows: The same ordered input of stake amounts `[4.20, 3.33, 12.00, 2.10, 1.05]` with the predicate `amount < 5`. Left, `filter`: output `[4.20, 3.33, 2.10, 1.05]`, every element tested. Right, `takeWhile`: output `[4.20, 3.33]`, traversal stops at 12.00 — the stop point marked and the untested elements greyed with a label
- **D-028 — Why `peek` may never run** · type `flowchart` · the file(s) already exist; embed exactly:
    ![D-028 — Why `peek` may never run](../diagrams/D-028-peek-may-never-run.svg)
  Beneath each embed put the caption line `**D-028** — Why `peek` may never run`. The diagram shows: Decision nodes for `count()`: is `SIZED` still set? did any stateful op clear it? does anything short-circuit? If all clear, the answer comes from the source's size and the sink chain is never built — the `peek` consumer boxed and marked "never called". A parallel branch shows the same pipeline with a `filter` added, clearing `SIZED`, and `peek` running. A VERSION TRAP banner: always ran before Java 9
- **D-029 — Operation order changes both the answer and the cost** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-029 — Operation order changes both the answer and the cost](../diagrams/D-029-operation-order-changes-both.svg)
  Beneath each embed put the caption line `**D-029** — Operation order changes both the answer and the cost`. The diagram shows: Left: `.sorted(byAmount).limit(10)` over 2.8M stake reservations — the full buffer and sort drawn, cost O(n log n), elements buffered = 2.8M. Right: `.limit(10).sorted(byAmount)` — a different answer, the ten elements shown. A third panel: `filter` before `map` versus after, with the number of mapper invocations counted for both
- **D-030 — `sorted()` is a barrier** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-030 — `sorted()` is a barrier](../diagrams/D-030-sorted-barrier.svg)
  Beneath each embed put the caption line `**D-030** — `sorted()` is a barrier`. The diagram shows: Frame 1: elements streaming into `sorted` and accumulating in its buffer, downstream stages idle. Frame 2: source exhausted, TimSort runs over the full buffer. Frame 3: elements released downstream. A side panel: a non-`Comparable` element producing `ClassCastException` at frame 2 — i.e. at terminal time, not at the `sorted()` call site
...
```
path: streams/03-intermediate-operations.md
