# Writer packet — `build-it/05-concurrency-builds.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/build-it/05-concurrency-builds.md
```
...
| Field | Value |
|---|---|
| File | `build-it/05-concurrency-builds.md` |
| Subject | build it |
| Part / tier | Part 4 — BUILD IT |
| Syllabus sections | §4.6 — Concurrency builds |
| Leaves | 4.6.1–4.6.8 (8 leaves) |
| Primary concepts | the echo server written twice and measured; a pinning reproducer and its `ReentrantLock` fix; a `ThreadLocal` memory harness; a `Semaphore`-bounded client; `ShutdownOnFailure` against `allOf` with a deliberate failure; a hedge; a common-pool starvation reproducer |
| Diagrams | D-173, D-174, D-175, D-176 |
| Examples (QuizStakes slice) | 1, 1,000 and 50,000 concurrent connections; a fan-out to the identity vendor and the watchlist provider with one deliberate failure |
| Previous | `build-it/04-records-sealed-patterns.md` |
| Next | `build-it/06-filling-the-21-gaps.md` |
| Target size | **828 lines**, and anything from 579 to 1117 is fine |
...
```
path: build-it/05-concurrency-builds.md
