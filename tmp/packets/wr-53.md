# Writer packet — `cost-model/02-master-tables.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/cost-model/02-master-tables.md
```
...
| Field | Value |
|---|---|
| File | `cost-model/02-master-tables.md` |
| Subject | cost model |
| Part / tier | Part 2 — INTERMEDIATE |
| Syllabus sections | §2.1 — The master tables |
| Leaves | 2.1.1–2.1.8 (8 leaves) |
| Primary concepts | the master stream cost table; the feature-by-version table; the lambda/method-reference/anonymous-class table; the absence-representation table; the data-carrier table; the concurrency-model table; the list-factory table |
| Diagrams | D-087, D-088, D-089, D-090, D-091, D-092, D-093 |
| Examples (QuizStakes slice) | Every cost quoted against 2.8M stake reservations and 95k card deposits per day |
| Previous | `library-additions/01-basics.md` |
| Next | `which-construct/02-which-construct.md` |
| Target size | **852 lines**, and anything from 596 to 1150 is fine |
...
These are the **8 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- **D-087 — The master stream cost table** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-087** — The master stream cost table` on the line beneath it. It must contain every row, column and value named here: One row per stream operation. Columns: per-element cost, allocations per stage, stateful, buffering (none / bounded / whole stream), parallel behaviour, amortised cost, worst case. Costs quoted against 2.8M stake reservations
- **D-088 — Feature by version, with its JEP and its trap** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-088** — Feature by version, with its JEP and its trap` on the line beneath it. It must contain every row, column and value named here: One row per feature this guide owns. Columns: feature, JEP number, first preview release, final release, what it replaced, and the one trap that ships with it
- **D-089 — Lambda vs method reference vs anonymous class vs inner class** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-089** — Lambda vs method reference vs anonymous class vs inner class` on the line beneath it. It must contain every row, column and value named here: Columns for each of the four. Rows: class files generated at compile time, classes created at runtime, allocations per evaluation, capture semantics, meaning of `this`, first-call linkage cost, serialization story, stack-trace readability, when it is the right answer
- **D-090 — Six ways to say "absent"** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-090** — Six ways to say "absent"` on the line beneath it. It must contain every row, column and value named here: Rows: `Optional`, `null`, a thrown exception, an empty collection, a null object, a sentinel value. Columns: caller must acknowledge, allocation cost, works in a field, works as a parameter, framework support, and the QuizStakes case each is correct for
- **D-091 — Five ways to carry data** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-091** — Five ways to carry data` on the line beneath it. It must contain every row, column and value named here: Columns: record, `final` class, enum, interface, `Map<String,Object>`. Rows: immutability, generated members, pattern deconstruction, extensibility, serialization, framework support, per-instance memory, when to choose
- **D-092 — Four concurrency models** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-092** — Four concurrency models` on the line beneath it. It must contain every row, column and value named here: Columns: platform threads, virtual threads, reactive (WebFlux/Reactor), structured concurrency. Rows: throughput ceiling, latency, stack traces, debugger, profiler, backpressure, cancellation, library support, team learning cost
- **D-093 — Seven ways to get a `List`** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-093** — Seven ways to get a `List`` on the line beneath it. It must contain every row, column and value named here: Rows: `new ArrayList<>()`, `Arrays.asList`, `List.of`, `List.copyOf`, `Collectors.toList`, `Collectors.toUnmodifiableList`, `Stream.toList`. Columns: mutable, structurally modifiable, `set` in place, nulls permitted, concrete type guaranteed, since which release
...
```
path: cost-model/02-master-tables.md
