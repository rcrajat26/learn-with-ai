# Writer packet — `streams/09-internals-spliterator.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/streams/09-internals-spliterator.md
```
...
| Field | Value |
|---|---|
| File | `streams/09-internals-spliterator.md` |
| Subject | streams |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.4 — `Spliterator` |
| Leaves | 3.4.1–3.4.14 (14 leaves) |
| Primary concepts | the eight characteristics with their hex bits; `SIZED` versus `SUBSIZED`; `trySplit` returning the prefix; the per-collection spliterators; the `IteratorSpliterator` batching fallback; writing one that splits well |
| Diagrams | D-135, D-136, D-137, D-138 |
| Examples (QuizStakes slice) | An `ArrayList` of 95,000 card deposits split 0–47,499 / 47,500–94,999; `LinkedList` and `Files.lines(paymentRunFile)` as the batching cases |
| Previous | `streams/08-internals-pipeline.md` |
| Next | `streams/10-internals-parallel-execution.md` |
...
- **D-135 — The eight spliterator characteristics** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-135** — The eight spliterator characteristics` on the line beneath it. It must contain every row, column and value named here: One row per characteristic with its hex bit value: `DISTINCT 0x01`, `SORTED 0x04`, `ORDERED 0x10`, `SIZED 0x40`, `NONNULL 0x100`, `IMMUTABLE 0x400`, `CONCURRENT 0x1000`, `SUBSIZED 0x4000`. Columns: meaning, which JDK sources report it, which stream optimisation it enables, and which operation clears it. The eight `Spliterator` methods listed in a legend
- **D-136 — `trySplit` returns the prefix** · type `step-sequence, 3 frames` · the file(s) already exist; embed exactly:
    ![D-136 — `trySplit` returns the prefix](../diagrams/D-136-trysplit-returns-prefix.svg)
  Beneath each embed put the caption line `**D-136** — `trySplit` returns the prefix`. The diagram shows: An `ArrayList` of 95,000 card deposits. Frame 1: one spliterator over indices 0–94,999. Frame 2: `trySplit` returns a spliterator over 0–47,499 (the prefix) and leaves the original covering 47,500–94,999 — both ranges labelled. Frame 3: the recursion continuing to a leaf below the target size. Index bounds written at every level
- **D-137 — `SIZED` but not `SUBSIZED`** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-137 — `SIZED` but not `SUBSIZED`](../diagrams/D-137-sized-but-subsized.svg)
  Beneath each embed put the caption line `**D-137** — `SIZED` but not `SUBSIZED``. The diagram shows: Left: an array-backed source — the total size is known and every split's size is known, so both flags are reported. Right: a balanced tree — the total is known (`SIZED`) but the subtree sizes are not (`not SUBSIZED`), drawn with the unknown subtree counts marked. The javadoc's own framing quoted in the caption
- **D-138 — Why an `Iterator`-derived stream parallelises badly** · type `step-sequence, 3 frames` · authored as **3 frame files**, and you must embed **all of them, in order**, each with its own caption line naming the frame · the file(s) already exist; embed exactly:
  ...
  Beneath each embed put the caption line `**D-138** — Why an `Iterator`-derived stream parallelises badly`. The diagram shows: `IteratorSpliterator`'s batching fallback. Frame 1: the first `trySplit` pulls a batch of 1024 elements into an array. Frame 2: the next batch doubles. Frame 3: the tail remains unsplittable and never reports `SUBSIZED`. Batch sizes written on each frame; `LinkedList` and `Files.lines` named as the cases



One consistency note: your diagram **D-136**'s inset works the `suggestTargetSize` arithmetic on an illustrative 5-core machine (`LEAF_TARGET = 4 << 2 = 16`, target 5,937), and says so on its face. The rest of this note set uses the 8-core convention in the verified-figures block (parallelism 7, `LEAF_TARGET` 28). Say in prose that the diagram's inset is a 5-core illustration and give the 8-core figures as the set's default, so the two never look like a contradiction.
...
```
path: streams/09-internals-spliterator.md
