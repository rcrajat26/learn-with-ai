# Writer packet — `optional/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/optional/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `optional/01-basics.md` |
| Subject | optional |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.11 — Optional |
| Leaves | 1.11.1–1.11.24 (24 leaves) |
| Primary concepts | the return-type-only purpose and the javadoc API note; value-based and not `Serializable`; the full method table by version; `orElse`'s eager argument; the `isPresent`+`get` anti-pattern; the four places it must never appear; `map`'s null-mapper behaviour |
| Diagrams | D-045, D-046, D-047, D-048 |
| Examples (QuizStakes slice) | `findClient(id)` chained `Client` to `Account` to `Wallet` to `Money.ZERO`; a `loadDefaultFromDatabase()` call counter proving eager evaluation |
| Previous | `collectors/03-internals-collectors.md` |
| Next | `optional/02-discipline.md` |
| Target size | **1020 lines**, and anything from 714 to 1377 is fine |
...
- **D-045 — `Optional`'s API by version** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-045** — `Optional`'s API by version` on the line beneath it. It must contain every row, column and value named here: One row per method across all 20. Columns: signature, release added (15 at 1.8, `ifPresentOrElse`/`or`/`stream` at 9, `orElseThrow()` at 10, `isEmpty` at 11), eager or lazy in its argument, what it does on empty, and whether it is on `OptionalInt`/`OptionalLong`/`OptionalDouble` too
- **D-046 — `orElse` evaluates eagerly even when the value is present** · type `step-sequence, 2 frames` · authored as **2 frame files**, and you must embed **all of them, in order**, each with its own caption line naming the frame · the file(s) already exist; embed exactly:
...
```
path: optional/01-basics.md
lines: <wc -l of the file you wrote>
leaves: 1.11.1–1.11.24
