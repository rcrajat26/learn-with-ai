# Writer packet — `lambdas/01-basics.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/lambdas/01-basics.md
```
...
| Field | Value |
|---|---|
| File | `lambdas/01-basics.md` |
| Subject | lambdas |
| Part / tier | Part 1 — BASICS |
| Syllabus sections | §1.3 — Lambda expressions |
| Leaves | 1.3.1–1.3.22 (22 leaves) |
| Primary concepts | lambda syntax forms; the poly expression and target typing; `this` and lexical transparency; capture by value and effectively-final; loop-variable capture; the recursion and checked-exception limits |
| Diagrams | D-009, D-010, D-011, D-012, D-013, D-014 |
| Examples (QuizStakes slice) | `BonusService` registering a `Runnable`; `FundsLedger.reserveStake` capturing a `Money stake`; iterating `reservations` versus a classic `for` index |
| Previous | `functional-interfaces/01-basics.md` |
| Next | `lambdas/02-cost-and-choice.md` |
| Target size | **1012 lines**, and anything from 708 to 1366 is fine |
...
These are the **22 syllabus leaves you own**, verbatim, in order. Every one must appear in the file, or be listed in `## Deferred` with a one-line reason. No other file covers them; no other file's leaves are yours.
...
- **D-009 — Every lambda syntax form** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-009** — Every lambda syntax form` on the line beneath it. It must contain every row, column and value named here: Rows: `() -> expr`, `x -> expr`, `(x, y) -> expr`, `(Type x) -> {...}`, `(var x) -> ...`, `(final @NonNull var x) -> ...`, block body with `return`. Columns: since which release, parameter typing (implicit/explicit/`var`), whether mixing is allowed, and a QuizStakes example for each
- **D-010 — A lambda is a poly expression** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-010 — A lambda is a poly expression](../diagrams/D-010-lambda-poly-expression.svg)
  Beneath each embed put the caption line `**D-010** — A lambda is a poly expression`. The diagram shows: The same source text `r -> r.amount().compareTo(MAX_STAKE) > 0` used at three sites with three different target types (`Predicate<Reservation>`, a custom `StakeRule`, a `Function<Reservation, Boolean>` that does *not* compile). Beside them: `Object o = () -> {};` marked "compile error: target type is not a functional interface" and `Object o = (Runnable) () -> {};` marked "compiles". Each site annotated with the target type that supplied the interface
- **D-011 — `this` in a lambda versus an anonymous class** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-011 — `this` in a lambda versus an anonymous class](../diagrams/D-011-this-lambda-versus-anonymous.svg)
  Beneath each embed put the caption line `**D-011** — `this` in a lambda versus an anonymous class`. The diagram shows: The same `Runnable` registered from inside `BonusService`, written twice. Left, anonymous class: `this` points at the anonymous instance, plus a synthetic `this$0` arrow to `BonusService`; generated name `BonusService$1`. Right, lambda: `this` points straight at the `BonusService` instance, no extra object; synthetic method `lambda$register$0`. Both arrows labelled
- **D-012 — Capture is by value, and only of effectively-final locals** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-012 — Capture is by value, and only of effectively-final locals](../diagrams/D-012-capture-value-only-effectively.svg)
  Beneath each embed put the caption line `**D-012** — Capture is by value, and only of effectively-final locals`. The diagram shows: A stack frame for `FundsLedger.reserveStake` holding `Money stake` (effectively final) and `this`; the spun lambda object on the heap with one field per captured value; an arrow from the field to the same `Money` object. A second panel: an instance field `dailyTotal` read inside the lambda — no field copy, only a `this` capture, so a later write to `dailyTotal` *is* visible. Both cases annotated with "copied at capture" / "read through `this` at invocation"
- **D-013 — Which loop variable is capturable** · type `before-after` · the file(s) already exist; embed exactly:
    ![D-013 — Which loop variable is capturable](../diagrams/D-013-loop-variable-capturable.svg)
  Beneath each embed put the caption line `**D-013** — Which loop variable is capturable`. The diagram shows: Left: `for (int i = 0; i < 3; i++)` — one variable, reassigned, so `i` is not effectively final and the lambda does not compile; the single slot drawn once with three values over time. Right: `for (Reservation r : reservations)` — a fresh variable per iteration, three separate slots, three lambdas each capturing its own. The compile error text quoted on the left
- **D-014 — Four ways to mutate from inside a lambda, and the one that is right** · type `table` · **render this as a Markdown table in your prose, at the point of explanation. There is no SVG and you must not create one.** Caption the table `**D-014** — Four ways to mutate from inside a lambda, and the one that is right` on the line beneath it. It must contain every row, column and value named here: Rows: one-element array hack, `AtomicInteger`, `reduce`, a collector, a plain loop. Columns: compiles, thread-safe in parallel, allocation cost, readability, verdict. The QuizStakes case is counting reservations over `STAKE_BLOCKED` clients
