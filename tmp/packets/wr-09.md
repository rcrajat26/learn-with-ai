# Writer packet — `lambdas/04-internals-capture-and-identity.md`
...
```
/Users/rajat.chikkodikar/Desktop/My-files/rough/src/notes/detailed/04-modern-java/lambdas/04-internals-capture-and-identity.md
```
...
| Field | Value |
|---|---|
| File | `lambdas/04-internals-capture-and-identity.md` |
| Subject | lambdas |
| Part / tier | Part 3 — INTERNALS |
| Syllabus sections | §3.2 — Lambda capture and identity |
| Leaves | 3.2.1–3.2.10 (10 leaves) |
| Primary concepts | capture by value into a spun field; capturing `this` versus capturing a field read; the listener-registry leak; lambda identity and why `==` is meaningless; what the JIT does with a lambda call site |
| Diagrams | D-130 |
| Examples (QuizStakes slice) | A static `NotificationService` registry holding a lambda that reads a `ProfileService` instance field, and the retained subgraph that follows |
| Previous | `lambdas/03-internals-translation.md` |
| Next | `method-references/01-basics.md` |
...
- **D-130 — A captured `this` keeps the enclosing object alive** · type `memory-layout` · the file(s) already exist; embed exactly:
    ![D-130 — A captured `this` keeps the enclosing object alive](../diagrams/D-130-captured-this-keeps-enclosing.svg)
  Beneath each embed put the caption line `**D-130** — A captured `this` keeps the enclosing object alive`. The diagram shows: A static `NotificationService` registry holding one lambda that reads an instance field of `ProfileService`. The captured `this` arrow drawn to the `ProfileService` instance, which retains its aggregated objects. Retained bytes labelled on the whole subgraph; the fix (capture only the needed value into a local) drawn beside it with the reduced retained set
...
```
path: lambdas/04-internals-capture-and-identity.md
