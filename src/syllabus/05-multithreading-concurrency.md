# Syllabus — 05 Multithreading and Concurrency
...
Primary sources are listed first within each group. Where a search returned nothing beyond what is
already in this file, that is stated rather than padded.

**Specification and API (primary)**

- <https://docs.oracle.com/javase/specs/jls/se21/html/jls-17.html> — JLS 21 chapter 17: the memory
  model (17.4.1–17.4.9), `wait`/`notify` (17.2), `sleep`/`yield` (17.3), final-field semantics
  (17.5), word tearing (17.6), 64-bit non-atomicity (17.7). Source of every `[SOURCE]` leaf in
  §1.10, §1.11, §1.12 and §3.7.
- <https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/package-summary.html>
  — the `j.u.c` package summary's memory-consistency properties list (§1.10.11) and the
  weakly-consistent-iterator definition (§1.16.5).
- <https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/locks/AbstractQueuedSynchronizer.html>
  — AQS as a CLH-queue variant with `prev`/`next` links plus a status field, exclusive and shared
  modes, the five template methods. Basis of §3.5.1–§3.5.12.
- <https://github.com/openjdk/jdk21/blob/master/src/java.base/share/classes/java/util/concurrent/locks/AbstractQueuedSynchronizer.java>
  — the JDK 21 source itself, confirming the post-JDK-14 rewrite (bit-flag status, `ExclusiveNode`/
  `SharedNode`/`ConditionNode`) rather than the JDK 8 `waitStatus` encoding most blogs describe.
  Basis of the `[VERSION-TRAP]` at §3.5.9.
- <https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/ForkJoinPool.html>
  — confirmed: common-pool system properties `common.parallelism`, `common.threadFactory`,
  `common.exceptionHandler`, `common.maximumSpares` (default 256); default parallelism =
  `availableProcessors()`; the ten-argument constructor with `corePoolSize`, `maximumPoolSize`
  (default `256 + parallelism` for the common pool), `minimumRunnable` (default 1), `saturate`,
  `keepAliveTime` (default 60 s); and the `ManagedBlocker` `isReleasable`/`block` contract.
  Basis of §1.22.8, §2.13, §3.11.9 and §3.11.13.

**JEPs (primary)**

- <https://openjdk.org/jeps/491> — Synchronize Virtual Threads without Pinning (Java 24). Direct
  fetch returned HTTP 403; the content was taken from the OpenJDK bug entry and secondary
  reporting below, and every constant sourced this way is tagged `[RESEARCH]` for re-verification
  in the write pass.
- <https://bugs.java.com/bugdatabase/view_bug?bug_id=8338813> — the implementation bug for JEP 491.
- <https://openjdk.org/jeps/374> — Deprecate and Disable Biased Locking (Java 15): the complexity
  and safepoint-revocation rationale behind §3.2.14.
- <https://openjdk.org/jeps/450> — Compact Object Headers (experimental, Java 24): headers from
  96–128 bits to 64 bits, klass pointer folded into the mark word. Basis of §3.1.6.

**Implementation and internals**

- <https://wiki.openjdk.org/display/HotSpot/Synchronization+Using+The+ObjectMonitorTable> — the
  mark-word states (neutral / stack-locked / inflated), the displaced header in the `BasicLock`,
  the `0b10` inflated tag, and the monitor-table redesign for compact headers. Basis of §3.1.3,
  §3.2.2–§3.2.6 and §3.2.13.
- <https://arxiv.org/pdf/2102.04188> — "Compact Java Monitors": `ObjectMonitor` structure
  (`_owner`, `_recursions`, `_EntryList`, `_cxq`, `_WaitSet`) and inflation costs. Basis of
  §3.2.7–§3.2.10.
- <https://github.com/openjdk/jdk14u/blob/master/src/java.base/share/classes/jdk/internal/vm/annotation/Contended.java>
  — `@Contended`'s package, the `-XX:-RestrictContended` requirement, and its use in `Striped64.Cell`.
  Basis of §3.9.5 and §3.9.8.
- <https://www.baeldung.com/java-false-sharing-contended> and <https://alidg.me/blog/2020/5/1/false-sharing>
  — the 64-byte cache line, the 128-byte `ContendedPaddingWidth` default, and a measurable
  false-sharing benchmark shape. Basis of §3.9.6–§3.9.11 and the §4.8.5 harness.
- <https://www.baeldung.com/java-memory-layout> and <https://nipafx.dev/inside-java-newscast-48/>
  — object header sizing (12 B with compressed oops, 16 B without) and the 10–20 % heap saving
  from compact headers. Basis of §3.1.1 and §3.1.6.
- <https://bugs.openjdk.org/browse/JDK-8315740> and
  <https://knowledge.broadcom.com/external/article?articleNumber=396719> (JDK-8330017) — two real
  `ForkJoinPool` failures: common-pool starvation, and the `ctl` release-count overflow at
  −32768 → +32767 that stopped a production pool entirely. Basis of §3.11.14–§3.11.15.
- <https://www.besthub.dev/articles/deep-dive-into-java-8-concurrenthashmap-initialization-treeification-resizing-and-transfer-mechanics-7fedb596a324>
  — `sizeCtl`'s four meanings, `initTable`'s CAS-to-−1, `addCount` → `CounterCell` → `fullAddCount`,
  and the strided cooperative `transfer`. Used as a pointer to the JDK source; every constant in
  §3.8.3–§3.8.4 must be re-read from `ConcurrentHashMap.java` during the write pass.

**Loom internals**

- <https://foojay.io/today/the-basis-of-virtual-threads-continuations/> — delimited continuations,
  `Continuation.yield`, and the `StackChunk` heap copy. Basis of §3.12.1–§3.12.7.
- <https://medium.com/@nikolaykudinov/inside-java-virtual-threads-a-deep-dive-into-the-jvm-implementation-part-1-0cf6793ac3bd>
  — `VirtualThread.runContinuation`, the mount/unmount cycle, the FIFO `ForkJoinPool` scheduler,
  and the poller-thread model for socket I/O. Basis of §3.12.3–§3.12.12.
- <https://www.infoq.com/articles/virtual-threads-after-jdk24/> — production deltas for JDK 24/25:
  `-Djdk.tracePinnedThreads` removed, `jdk.VirtualThreadPinned` 20 ms threshold,
  `jcmd Thread.dump_to_file -format=json`, residual pinning (native/JNI/FFM, class loading, Linux
  file I/O with no io_uring), the measured `ThreadLocal` initialisation regression
  (200 → 443 267), downstream resource exhaustion as the dominant failure mode, and
  `ScopedValue.orElse(null)` no longer permitted in the finalised JEP 506 API. Basis of §2.9 and
  §3.12.13–§3.12.17.
- <https://nljug.org/foojay/your-loom-app-quietly-became-a-thread-pool-again-a-field-guide-to-virtual-thread-pinning/>
  and <https://www.javacodegeeks.com/2026/05/virtual-threads-two-years-in-production-war-stories-the-pinning-edge-cases-and-what-jdk-25-fixed.html>
  — the all-carriers-pinned pseudo-deadlock, the CLOSE_WAIT signature, and the "jstack shows an
  idle JVM" diagnostic dead end. Basis of §2.9.4–§2.9.6.
- <https://mikemybytes.com/2025/04/09/java24-thread-pinning-revisited/> and
  <https://www.danvega.dev/blog/jdk-24-virtual-threads-without-pinning> — confirmation of the
  JDK 24 behaviour change and the removed diagnostic flag.

**Safepoints and JIT**

- <https://blanco.io/blog/jvm-safepoint-pauses/> and
  <https://www.javacodegeeks.com/2026/05/reading-jvm-safepoint-logs-without-going-mad-a-practical-stop-the-world-diagnosis-guide.html>
  — TTSP as distinct from pause duration, `-Xlog:safepoint*`, and the causes of long TTSP. Basis
  of §3.4.3 and §3.4.8.
- <https://technology-dhami.blogspot.com/2016/11/java-vm-safepoint-for-revokebias.html> — the
  `RevokeBias` safepoint, the historical cost that JEP 374 removed. Basis of §3.4.5.
- The `GuaranteedSafepointInterval` default of 1000 ms and the
  `-XX:+UnlockDiagnosticVMOptions -XX:GuaranteedSafepointInterval=0` override, confirmed in the
  safepoint-logging sources above. Basis of §3.4.4.
- <https://jpbempel.github.io/2022/06/22/debug-non-safepoints.html> — safepoint bias in profilers
  and `-XX:+DebugNonSafepoints`. Basis of §3.4.9.

**Lock algorithms and lock-free structures**

- <https://www2.cs.sfu.ca/~tzwang/mcsg.pdf> — MCS lock structure and its relationship to CLH and
  ticket locks. Basis of §4.1.4–§4.1.7.
- <https://ethancornell.github.io/blog/2025/LFQ_EBR/> and
  <https://aturon.github.io/blog/2015/08/27/epoch/> — hazard pointers versus epoch-based
  reclamation, and the statement that the Michael–Scott queue is what `ConcurrentLinkedQueue`
  implements. Basis of §3.10.1, §4.4.3–§4.4.6.

**Testing**

- <https://openjdk.org/projects/code-tools/jcstress/> and <https://github.com/openjdk/jcstress>
  — the litmus-test harness, its purpose (JMM conformance), and the honest framing that most
  application developers never need it. Basis of §2.12.7–§2.12.8 and §4.8.9–§4.8.10.

**Interview surface**

- <https://www.educative.io/blog/java-multithreading-and-concurrency-what-to-know-for>,
  <https://interviewkickstart.com/blogs/interview-questions/java-concurrency-interview-questions>,
  <https://interviewquestions.guru/java-multithreading-interview-questions-answers/> — mined
  purely for question names not already in §5.1. They contributed the dining-philosophers,
  alternating-printer, and "design a connection pool" implementation questions and confirmed that
  virtual threads and `CompletableFuture` are now standard senior-level material.

**Not found / not usable**

- A direct fetch of <https://openjdk.org/jeps/491> returned HTTP 403, as did repeated attempts at
  the JEP index. Every JEP-derived constant in this file therefore carries `[RESEARCH]` and must
  be re-verified in the write pass, ideally from the JDK release notes or the `jdk` repository
  rather than from secondary reporting.
- No primary source was found for a current, authoritative per-instruction cost table for
  park/unpark on Linux; the numbers in §2.1.2 and §3.6.6 are order-of-magnitude and must be
  presented as such rather than as measured constants.

## Gaps vs the current guide

`src/topics/05-multithreading-concurrency.md` is 573 lines and covers 17 sections. The table below
is the work order: every row marked *missing* is content the bible must add, every row marked
*shallow* is content that exists but at one to three lines where the syllabus demands mechanism,
numbers or proof.

| Syllabus area | Present in `src/topics/05-…md` | Missing | Shallow |
|---|---|---|---|
| §1.1 why concurrency exists, Amdahl, Little, USL | — | ✅ entire section | |
| §1.2 process/thread, share-own split | §1 | | ✅ no context-switch numbers, no footprint arithmetic, no ulimit/thread-limit story |
| §1.3 the `Thread` API surface | §2 (partial) | ✅ Builder API, `ofVirtual`, `onSpinWait`, removed members, `holdsLock`, `ThreadFactory` | ✅ `start()` twice states the **wrong exception** (`IllegalStateException`; it is `IllegalThreadStateException`) — must be corrected |
| §1.4 lifecycle and states | §2 | ✅ transition diagram, virtual-thread states, `getState` is advisory | |
| §1.5 interruption and cancellation | §2 (4 lines) | ✅ the two legal responses, the interruptible-blocking-point inventory, non-interruptible points, poison pills, shutdown hooks, deadline propagation | ✅ |
| §1.6 thread-safety vocabulary | — | ✅ entire section (taxonomy, confinement, `@GuardedBy`, escaping, alien methods) | |
| §1.7 races and compound actions | §3 | ✅ race vs data race, 64-bit tearing, word tearing, TOCTOU, the Java 7 HashMap loop | ✅ |
| §1.8 `synchronized` | §4 | ✅ bytecode (`monitorenter` vs `ACC_SYNCHRONIZED`), JLS 17.4.4 citation, class-init lock, virtual-thread pinning row | |
| §1.9 `volatile` | §5 | ✅ the "flushes to main memory" myth is **repeated in the current guide** and must be corrected, plus array-reference trap, cost asymmetry, `lazySet` | |
| §1.10 the JMM formally | §6 (18 lines) | ✅ JLS 17.4.1–17.4.9, action tuples, synchronizes-with, causality/OOTA, DRF-SC, barrier categories, roach motel, `println` accident | ✅ severely |
| §1.11 final fields, safe publication, DCL | §13 | ✅ freeze action, dereference chains, 17.5.2–17.5.4, the four `this`-escape routes, holder-idiom proof, class-init deadlock, `StableValue` | ✅ |
| §1.12 `wait`/`notify` | §15 | ✅ JLS 17.2 wait-set semantics, lost-wakeup, thundering herd, timed-wait cannot report timeout | |
| §1.13 atomics, CAS, `VarHandle` | §7 | ✅ the 16-class inventory, full method surfaces, `weakCompareAndSet`, `VarHandle` access modes and the four ordering levels, fences, `Unsafe` removal, `ThreadLocalRandom` | ✅ |
| §1.14 explicit locks | §16 | ✅ `tryLock()` vs `tryLock(0,unit)` barging, instrumentation methods, downgrade/upgrade, `StampedLock` full surface and its four traps, `LockSupport` permits | ✅ |
| §1.15 synchronizers | §16 table | ✅ full method surfaces, broken barrier, arrival index, `Phaser` tiering, per-synchronizer happens-before edges | ✅ |
| §1.16 concurrent collections | §14 (CHM only) | ✅ the 15-class inventory, the three iterator models, skip lists, CoW cost model and its two traps, `ConcurrentLinkedQueue`, views/copies/snapshots | ✅ |
| §1.17 `BlockingQueue` | §11 | ✅ the four-family grid as a table, `drainTo`, the two-lock proof, `LinkedTransferQueue`, `BlockingDeque` | ✅ |
| §1.18 the Executor framework | §9 | ✅ interface stack, `AutoCloseable` in 19, `submit` vs `execute` exception loss, `invokeAll`/`invokeAny`, `CompletionService`, `Future.state` (19), `FutureTask` | ✅ |
| §1.19 `ThreadPoolExecutor` | §9 | ✅ the javadoc's three queuing strategies, custom rejection handlers, dynamic knobs, hooks, monitoring surface, task-dependency deadlock, container CPU limits | ✅ sizing formula is one line and undertived |
| §1.20 `ScheduledThreadPoolExecutor` | §9 (3 lines) | ✅ why the cancel is silent, `setRemoveOnCancelPolicy` leak, `Timer` obsolescence, distributed-scheduler caveat | ✅ |
| §1.21 `CompletableFuture` | §10 | ✅ 18 combination methods, `handle`/`whenComplete`/`exceptionally` table, cancel semantics, timeouts, Java 9 additions, subclassing, `CompletionStage` as the API type | ✅ |
| §1.22 fork/join | — | ✅ entire section | |
| §1.23 `ThreadLocal` | §12 | ✅ classloader leak, virtual-thread interaction, context propagation across executors, MDC/tracing | ✅ |
| §1.24 virtual threads (mechanism) | — (deferred to 04) | ✅ entire section — the index says 05 owns concurrency mechanism, so pinning internals, scheduler sizing, JFR events and the JSON dump belong here | |
| §1.25 structured concurrency / scoped values | — | ✅ entire section, including the JEP 505/506 version deltas | |
| §1.26 liveness failures | §8 | ✅ dynamic lock ordering, open calls, resource deadlock, lock convoy detail, `ThreadMXBean` detection, what the detector cannot see | ✅ |
| PART 2 — master cost/latency/footprint tables | — | ✅ | |
| PART 2 — contention economics, striping, USL | — | ✅ | |
| PART 2 — primitive-choice decision tables | partial (§16 table) | ✅ | ✅ |
| PART 2 — pool sizing derivation, queue sizing, container limits | §9 (one line) | ✅ | ✅ |
| PART 2 — concurrent-collection choice | — | ✅ | |
| PART 2 — backpressure design beyond the queue | §11 | ✅ shedding, batching, ordering, graceful shutdown | ✅ |
| PART 2 — `CompletableFuture` in production | §10 | ✅ context propagation, retry, bounded parallelism, debuggability | ✅ |
| PART 2 — virtual threads in production | — | ✅ | |
| PART 2 — thread-safe class design | — | ✅ | |
| PART 2 — testing concurrency (jcstress, JMH, ErrorProne) | — | ✅ | |
| PART 2 — the utility surface (`TimeUnit`, `ThreadMXBean`, `Flow`, nanoTime) | — | ✅ | |
| PART 2 — distributed analogues | — | ✅ | |
| PART 2 — version delta 5 → 25 | — | ✅ | |
| PART 3 — mark word, header, compact headers | — | ✅ | |
| PART 3 — thin locks, inflation, `ObjectMonitor`, biased-locking history | — | ✅ | |
| PART 3 — lock elision/coarsening, volatile codegen, x86-TSO vs AArch64 | §6 (one clause) | ✅ | ✅ |
| PART 3 — safepoints | — | ✅ | |
| PART 3 — AQS | — | ✅ | |
| PART 3 — `LockSupport`/park internals | — | ✅ | |
| PART 3 — JMM formalism, litmus tests, cookbook | §6 | ✅ | ✅ |
| PART 3 — CHM internals (constants, `sizeCtl`, transfer, counting) | §14 | ✅ | ✅ — has the per-bin idea, none of the mechanism |
| PART 3 — `Striped64` and false sharing | §7 (two lines on LongAdder) | ✅ | ✅ |
| PART 3 — queue and executor internals (`ctl`, `Worker`, `FutureTask`, CF Treiber stack) | — | ✅ | |
| PART 3 — `ForkJoinPool` internals | — | ✅ | |
| PART 3 — virtual thread internals | — | ✅ | |
| PART 3 — runtime observability (dump anatomy, JFR events, nid join) | §8 (one line on jstack) | ✅ | ✅ |
| PART 4 — every `[BUILD]` | §17 producer–consumer only | ✅ all of §4.1–§4.8 except the producer–consumer assembly | ✅ |
| PART 5 — the 132-question set | — | ✅ | |
| PART 5 — the 55-item trap index | ~14 `**Trap:**` markers inline | ✅ all must be preserved and the rest added | |
| PART 5 — numbers/table/code/diagnosis drills | closing checklist (30 lines) | ✅ the drills | ✅ the checklist must be preserved and extended |

Two corrections the write pass **must** make to existing text, not merely additions:

1. §2 of the current guide states `start()` twice throws `IllegalStateException`. It throws
   `IllegalThreadStateException`.
2. §5 of the current guide says volatile reads and writes "go to main memory (conceptually), never
   to a thread-local cached copy". This is the cache-flush myth. Restate it in happens-before
   terms, then describe the store-buffer/invalidate-queue reality (§1.9.5).
...
Target version restated for the write pass: **Java 21 LTS**, with every Java 22–25 divergence
marked `[VERSION-TRAP]` inline. The two deltas that most often produce a stale answer are
**JEP 491 (Java 24): `synchronized` no longer pins virtual threads** and **JEP 506 (Java 25):
scoped values final while structured concurrency remains preview**.
